const char *rcsid_gtk_gx11_c =
    "$Id: gx11.c,v 1.1.1.1 2012/09/14 03:30:04 serpentshard Exp $";
/*
...
#include "item.h"
#include "pixmaps/wograldtitle.xpm"
#include "gx11.h"
#include "gtkproto.h"
#include <script.h>
#include <p_cmd.h>
...
#include "help/about.h"
  ...
  if(!gtkwin_about) {
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_about), "About Wograld");
...
#include "help/bugreport.h"
  ...
  if(!gtkwin_bug) {
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_bug), "Report a bug in Wograld");
...
#include "help/shelp.h"
  ...
  if(!gtkwin_shelp) {
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_shelp), "Wograld Server Help");
...
static void create_splash(void) {
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_splash), "Welcome to Wograld");
...
void create_windows(void) {
  ...
  if (want_config[CONFIG_SPLITWIN]==FALSE) {
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_root), "Wograld GTK Client");
  ...
  /* game window */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_root), "Wograld - view");
  ...
  /* Stats and menu window */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_stats), "Wograld GTK Client");
   ...
   /* info window - text and messages */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_info), "Wograld - info");
 ...
 /* statbars window */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_message), "Wograld - vitals");
 ...
 /* inventory window */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_inv), "Wograld - inventory");
 ...
 /* look window */
    ...
    gtk_window_set_title (GTK_WINDOW (gtkwin_look), "Wograld - look");
...
{
    /*
     * TODO Have wograld-server send paragraphs rather than lines, so to
...
void save_winpos(void)
{
	...
	sprintf(savename,"%s/.wograld/gwinpos", getenv("HOME"));
    else
...
void set_window_pos(void)
{
	...
	sprintf(buf,"%s/.wograld/gwinpos", getenv("HOME"));
 ...
 */
     ...
     */
    init_client_vars();
    snprintf( file_cache, MAX_BUF, "%s/.wograld/servers.cache", getenv( "HOME" ) );
