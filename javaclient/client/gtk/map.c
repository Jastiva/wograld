const char *rcsid_gtk_map_c =
    "$Id: map.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $";
/*
...
static void display_mapcell(int ax, int ay, int mx, int my)
{
    ...
    if (layer==MAXLAYERS)
        gdk_draw_rectangle(mapwindow, drawingarea->style->black_gc, TRUE, 0.5*map_image_size*(ax + ay - 10), 
0.5*map_image_size*(ay - ax + 7), map_image_size, map_image_size);
	...
	for (layer=0; layer<MAXLAYERS; layer++) {
		...
		draw_pixmap(
		    ...
		    0.5*map_image_size*(ax + ay - 10)+map_image_size-w, 0.5*map_image_size*(ay - ax + 7)+map_image_size-h,
	    ...
	    /* face = mapdata_bigface(ax, ay, layer); */
	    if (face > 0 && pixmaps[face]->map_image != NULL) {
		draw_pixmap(
		    sx*map_image_size, sy*map_image_size,
		    ax*map_image_size, ay*map_image_size,
		    0.5*map_image_size*(ax - sx + ay - sy - 10),0.5*map_image_size*(ay - sy - ax + sx + 7) ,
		    pixmaps[face]->map_mask, pixmaps[face]->map_image);
	    }
     ...
     */
    if (use_config[CONFIG_FOGWAR] && the_map.cells[mx][my].cleared) {
	draw_pixmap(0, 0, ax*map_image_size, ay*map_image_size, 0.5*map_image_size*(ax + ay - 10), 
	0.5*map_image_size*(ay - ax + 7), dark1, dark);
    }
	...
	draw_pixmap(0, 0, ax*map_image_size, ay*map_image_size, 0.5*map_image_size*(ax + ay - 10), 
	0.5*map_image_size*(ay - ax + 7), dark2, dark);
    } else if (the_map.cells[mx][my].darkness> 1) {
	draw_pixmap(0, 0, ax*map_image_size, ay*map_image_size, 0.5*map_image_size*(ax + ay - 10), 
	0.5*map_image_size*(ay - ax + 7), dark3, dark);
    }
}
