...
*/
  ...
  /*public static void main(String[] arg){
    ...
    frame.setVisible(true);
     ...
     public void launchOgg(String[] arg){
   ...
   if(arg.length>0){
    ...
    frame.setVisible(true);
     ...
     void clearPlaylist(JOrbisPlayer pr)
     {
         // find out how to stop the playback
         // then remove all elements
            pr.playlist.removeAllElements();
          // then enter the new track
          // then resume play
          // at the moment no memory of time in the track is needed
     }
     
     public void changeTrack(JOrbisPlayer pr, int trackNum )
     {
         pr.stop_sound();
         ...
         pr.running_as_applet=false;
       // use a track number because
       // it is likely that names can be corrupted in network transfer
       // and that longer names can be corrupted more easily
       // and in either case have a fallback case
      if(trackNum==0)
      {
        pr.playlist.addElement("resource/music/00.ogg");
      }
      else
      {
            pr.playlist.addElement("resource/music/gmajor.ogg");
      }
        pr.loadPlaylist();
       pr.play_sound();
