const char *rcsid_gtk_image_c =
    "$Id: image.c,v 1.1.1.1 2012/09/14 03:30:04 serpentshard Exp $";
/*
...
int create_and_rescale_image_from_data2(int pixmap_num, uint8 *rgba_data, int width, int height)
{
    ...
    if (pixmaps[pixmap_num+10000] != pixmaps[0]) {
	free_pixmap(pixmaps[pixmap_num+10000]);
	free(pixmaps[pixmap_num+10000]);
	pixmaps[pixmap_num+10000] = pixmaps[0];
    }
...
void image_update_download_status(int start, int end, int total)
{
	...
	/* we more or less want this window centered on the main wograld window,
