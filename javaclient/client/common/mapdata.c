/* $Id: mapdata.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $ */
/*
...
/* printf("%i %i %i\n",cell->heads[3].face,cell->heads[4].face,cell->heads[5].face);*/
...
void mapdata_set_face(int x, int y, int darkness, sint16 face0, sint16 face1, sint16 face2, int floor)
{
    int px, py;
    sint16 face[3];
    int is_blank;
 ...
 /*   face[0 + 3*floor] = face0; */
 /*   face[1 + 3*floor] = face1; */
 /*   face[2 + 3*floor] = face2; */
face[0] = face0;
face[1] = face1; 
face[2] = face2; 
    ...
    if (x < width && y < height) {
        ...
        if (is_blank) {
            ...
            if (!the_map.cells[px][py].cleared) {
                ...
                expand_need_update_from_layer(px, py, 3*floor);
                expand_need_update_from_layer(px, py, 3*floor + 1);
                expand_need_update_from_layer(px, py, 3*floor + 2);
            }
        }
        else {
            ...
            if (the_map.cells[px][py].cleared) {
                ...
                expand_clear_face_from_layer(px, py, 3*floor);
                expand_clear_face_from_layer(px, py, 3*floor +1);
                expand_clear_face_from_layer(px, py, 3*floor +2);
                the_map.cells[px][py].darkness = 0;
                the_map.cells[px][py].have_darkness = 0;
            }
           /* for (i = 3*floor; i < (3*floor + 3); i++) */ 
		for (i = 0; i < 3; i++) {
                if (face[i] != -1) {
		...
		/*	printf("%i\n",floor); */
			...
			expand_set_face(px, py, (i + 3*floor), face[i]);
   ...
   /*     for (i = 3*floor; i < (3*floor + 3); i++)*/
 		for (i = 0; i < 3; i++) {
            if (is_blank || face[i] != -1) {
                expand_set_bigface(x, y, (i + 3*floor), is_blank ? 0 : face[i]);
...
sint16 mapdata_bigface(int x, int y, int layer)
{ /* , int *ww, int *hh */
    sint16 result;
