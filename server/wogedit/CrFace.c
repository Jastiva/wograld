...
int DrawObject(Widget wd, int x, int y, object * tmp)
{
...
static int DrawObject2(Widget wd, int x, int y, object * tmp, int pixlx, int pixly)
{
    CrFaceWidget w = (CrFaceWidget)wd;
    ...
    if (!tmp)
     ...
     DrawPartObject2(wd, w->crFace.gc, tmp, x*FontSize, y*FontSize,pixlx,pixly,0);
     return(0);
}

static int DrawObject3(Widget wd, int x, int y, object * tmp, int pixlx, int pixly)
{
    CrFaceWidget w = (CrFaceWidget)wd;
    ...
    if (!tmp)
      ...
      DrawPartObject2(wd, w->crFace.gc, tmp, x*FontSize, y*FontSize, pixlx, pixly,1);
      return(0);
...
debug("Redisplay()\n");
  if(self->crFace.ob == NULL){
	...
	for(obj = self->crFace.ob; obj; obj = obj->more){
		DrawObject (w, obj->x - minx - x, obj->y - miny - y, obj);
	}
}
/*  CrFaceWidget self = (CrFaceWidget)w;
  object *obj;
