 ...
 *    "$Id: request.c,v 1.23 2015/06/06 17:41:12 serpentshard Exp $";
 */
...
//		return;
		...
		char tmpbuf[2];
		tmp=atoi(param);
		// tmp = NUM_LOOK_OBJECTS;
		sprintf(tmpbuf,"%d",tmp);
		safe_strcat(cmdback,tmpbuf,&slen,HUGE_BUF); 
		// at this time, server will not modify the num_look_objects
	} else {
...
void send_fcnm(int nmbr, char name, socket_struct *ns){
	...
	// sl.buf = reply; 
	strcpy((char*)sl.buf,"fcnmcmd ");
	sl.len=strlen((char*)sl.buf);
	SockList_AddShort(&sl, nmbr);
/*	SockList_AddChar(&sl, ' ');*/
/*	for (i = 0; name[i] != ' '; i++)*/
/*	{ */
/*		SockList_AddChar(&sl, name[i]);*/
/*	}*/
	SockList_AddChar(&sl,name);
	SockList_AddChar(&sl, ' ');
	Send_With_Handling(ns, &sl);
}

void send_enfl(int nmbr, socket_struct *ns){
...
void send_dsfl(int nmbr, socket_struct *ns){
	...
	// sl.buf = reply; 
	strcpy((char*)sl.buf,"dsflcmd ");
	sl.len=strlen((char*)sl.buf);
	SockList_AddInt(&sl, nmbr);
...
void map_newmap_cmd( player *pl)
{
	...
	memset(&pl->socket.upper1,0,sizeof(pl->socket.upper1));
	memset(&pl->socket.upper2,0,sizeof(pl->socket.upper2));
	memset(&pl->socket.upper3,0,sizeof(pl->socket.upper3));
	memset(&pl->socket.lower1,0,sizeof(pl->socket.lower1));
	memset(&pl->socket.lower2,0,sizeof(pl->socket.lower2));
	memset(&pl->socket.lower3,0,sizeof(pl->socket.lower3));
...
static object  *heads[MAX_HEAD_POS * MAX_HEAD_POS * MAP_LAYERS * 7];
 ...
 */
static inline int have_head(int ax, int ay, int floor) {

    if (heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) ] ||
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + 1] ||
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + 2]) return 1;
    return 0;
}
 ...
 */
static inline int check_head(SockList *sl, socket_struct *ns, int ax, int ay, int layer, int floor)
{
    short face_num;

    if (heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer])
	face_num = heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer]->face->number;
    else
	face_num = 0;
	...
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
...
static inline int check_head2(SockList *sl, socket_struct *ns, int ax, int ay, int layer, int floor)
{
    short face_num;
	...
	{
if (face_num != ns->lower1.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->lower1.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
	...
	{
if (face_num != ns->lower2.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->lower2.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
	...
	{
if (face_num != ns->lower3.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->lower3.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
	...
	{
if (face_num != ns->upper1.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->upper1.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
	...
	{
if (face_num != ns->upper2.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->upper2.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
	...
	{
if (face_num != ns->upper3.cells[ax][ay].faces[layer]) {
	SockList_AddShort(sl, face_num);
	if (face_num && !(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    esrv_send_face(ns, face_num, 0);
	heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor) + layer] = NULL;
	ns->upper3.cells[ax][ay].faces[layer] = face_num;
	return 1;
    }
	}
...
static inline int check_head3(SockList *sl, socket_struct *ns, int ax, int ay, int layer, int floor)
{
    short face_num;
    ...
    if (face_num != ns->lastmap.cells[ax][ay].faces[layer]) {
// fixup to use map2 like protocol
int nlayer = 0x10 + layer;
...
static inline int check_head4(SockList *sl, socket_struct *ns, int ax, int ay, int layer, int floor)
{
    short face_num;
...
nlayer |=  2 << 5;
        ...
        {
// I am not sure why, but this block was commented before
    if (face_num != ns->lower3.cells[ax][ay].faces[layer]) {
int nlayer = 0x10 + layer;
nlayer |=  2 << 5;
            SockList_AddChar(sl, nlayer);
 ...
 * draw for this space.
 */
static uint16 add_head(object *ob, int sx, int sy, int p_layer, int floor)
{
    object *head;
    int bx, by, i, cx, cy;
    uint16 face_num=0;
...
/* }} */
	...
	for (i=0; i<MAP_LAYERS; i++)
	    if (heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + i] &&
		heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + i]->face->number == face_num)
		heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + i] = NULL;
    }
    else {
	...
	for (i=0; i<MAP_LAYERS; i++)
	    if (heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i] &&
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i]->face->number == face_num)
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i] = NULL;

	/* First, try to put the new head on the same layer.  If that is used up,
	 * then find another layer.
	 */
	if (heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + p_layer] == NULL) {
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + p_layer] = head;
	} else for (i=0; i<MAP_LAYERS; i++) {
	    if (heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i] == NULL ||
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i] == head) {
		    heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + i] = head;
	    }
	}
...
static uint16 add_head2(object *ob, int sx, int sy, int p_layer, int floor)
{
    object *head;
    int bx, by, i, cx, cy;
    uint16 face_num=0;
...
static inline int update_space(SockList *sl, socket_struct *ns, mapstruct  *mp, int mx, int my, 
			       int sx, int sy, int m_layer, int p_layer, int floor)
{
    object *ob, *head;
    uint16 face_num;
    int bx, i;
    ...
    head = heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + p_layer];
	...
	for (i=0; i<MAP_LAYERS; i++) {
	    ...
	    if (ob == head) {
		heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS * (1+floor) + p_layer] = NULL;
		head = NULL;
	    }
	}
    }
	...
	if ((ob->arch->tail_x || ob->arch->tail_y) ||
	    ...
	    face_num=add_head(ob, sx, sy, p_layer, floor);
		...
		if (heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS * (1+floor) + bx] &&
		    heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS * (1+floor) + bx]->face->number == face_num)
			heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS * (1+floor) + bx] = NULL;
	}
     ...
     */
    if (!face_num && p_layer > 0 && heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + p_layer -1]) {
	face_num = heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + p_layer -1]->face->number;
	heads[(sy * MAX_HEAD_POS + sx) * MAP_LAYERS* (1+floor) + p_layer -1] = NULL;
    }
...
if (floor == 1){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->lower1.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->lower1.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->lower1.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    		esrv_send_face(ns, face_num, 0);
			SockList_AddShort(sl, face_num);
			return 1;
...
if (floor == 2){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->lower2.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->lower2.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->lower2.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    		esrv_send_face(ns, face_num, 0);
			SockList_AddShort(sl, face_num);
			return 1;
...
if (floor == 3){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->lower3.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->lower3.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->lower3.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
	    		esrv_send_face(ns, face_num, 0);
			SockList_AddShort(sl, face_num);
			return 1;
...
if (floor == 4){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->upper1.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->upper1.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->upper1.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
			esrv_send_face(ns, face_num, 0);
...
if (floor == 5){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->upper2.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->upper2.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->upper2.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
			{
	    			esrv_send_face(ns, face_num, 0); 
...
if (floor == 6){
 		...
 		if (face_num && p_layer+1<MAP_LAYERS && ns->upper3.cells[sx][sy].faces[p_layer+1] == face_num) 
		{
		face_num = 0;
		}
		if (ns->upper3.cells[sx][sy].faces[p_layer] != face_num) 
		 {
			ns->upper3.cells[sx][sy].faces[p_layer] = face_num;
			if (!(ns->faces_sent[face_num] & NS_FACESENT_FACE))
			{
			if(!((mx + my)%5))
				{
					if( ( sx > 9)|| (sy < 9) )
					{
	    					esrv_send_face(ns, 4491, 0);
					}
					else
					{
						esrv_send_face(ns,blank_face->number, 0);
					}
				}
			   else {
				esrv_send_face(ns,blank_face->number, 0);
		...
		/*	SockList_AddShort(sl, face_num); */
			if(!((mx + my)%5)){
					if( ( sx > 9)|| (sy < 9) )
					{
	    					SockList_AddShort(sl, 4491);
					}
					else
					{
						SockList_AddShort(sl,blank_face->number);
					}
				}
			   else {
					SockList_AddShort(sl, blank_face->number);
...
static inline int update_space2(SockList *sl, socket_struct *ns, mapstruct  *mp, int mx, int my, 
			       int sx, int sy, int m_layer, int p_layer, int floor)
{
    object *ob, *head;
    uint16 face_num;
	...
	if ((ob->arch->tail_x || ob->arch->tail_y) ||
	    ...
	    face_num=add_head2(ob, sx, sy, p_layer, floor);
...
static inline int update_space3(SockList *sl, socket_struct *ns, mapstruct  *mp, int mx, int my,
                               int sx, int sy, int m_layer, int p_layer, int floor)
{
    object *ob, *head;
    uint16 face_num;
...
// emulate map2 protocol
int nlayer = 0x10 + p_layer;
...
static inline int update_space4(SockList *sl, socket_struct *ns, mapstruct  *mp, int mx, int my,
                               int sx, int sy, int m_layer, int p_layer, int floor)
{
    object *ob, *head;
    uint16 face_num;
 ...
 ob = head;
    ...
    if (!ob || ob->face == blank_face) face_num=0;
        ...
        if ((ob->arch->tail_x || ob->arch->tail_y) ||
            ...
            face_num=add_head(ob, sx, sy, p_layer, floor);
// I am not sure why, but update_space2 calls add_head2
// Is add_head2 actually different from add_head ?
...
//    sl.buf=malloc(MAXSOCKSENDBUF);
    ...
    /* Init data to zero */
    memset(heads, 0, sizeof(object *) * MAX_HEAD_POS * MAX_HEAD_POS * MAP_LAYERS * 7);
...
if(pl->contr->socket.norm_tiles == 0)
{
	esrv_send_face(&pl->contr->socket, dark_faces[0]->number, 0);
	esrv_send_face(&pl->contr->socket, dark_faces[1]->number, 0);
	esrv_send_face(&pl->contr->socket, dark_faces[2]->number, 0);
	esrv_send_face(&pl->contr->socket, blank_face->number, 0);
	esrv_send_face(&pl->contr->socket, blocked_face->number, 0);

send_fcnm(blank_face->number, "a", &pl->contr->socket); 
send_fcnm(blocked_face->number, "b", &pl->contr->socket);
send_fcnm(dark_faces[0]->number, "c", &pl->contr->socket);
send_fcnm(dark_faces[1]->number, "d", &pl->contr->socket);
send_fcnm(dark_faces[2]->number, "e", &pl->contr->socket);

pl->contr->socket.norm_tiles = 1;
}
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head(&sl, &pl->contr->socket, ax, ay, 2, 0))
		    mask |= 0x4;
		if (check_head(&sl, &pl->contr->socket, ax, ay, 1, 0))
		    mask |= 0x2;
		if (check_head(&sl, &pl->contr->socket, ax, ay, 0, 0))
		    mask |= 0x1;
		...
		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 0)) {
...
/*printf("%s\n",m->path);*/
		...
		/* Floor face */
		if (update_space(&sl, &pl->contr->socket, m, nx, ny, ax, ay, b_layer, 2, 0))
		    mask |= 0x4;
		...
		/* Middle face */
		if (update_space(&sl, &pl->contr->socket, m, nx, ny, ax, ay, m_layer, 1, 0))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space(&sl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0, 0))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
...
void draw_upper_map1(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
    ...
    uint8 eentrysize, extendedinfos;
    mapstruct *m, *mu1;
    object *m_ob, *t_ob, *ob, *head2;

    // sl.buf=malloc(MAXSOCKSENDBUF);
   
	strcpy((char*)sl.buf,"upper1 ");
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 4))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 4))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 4))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.upper1.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask); 
		    map_clearcell(&pl->contr->socket.upper1.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.upper1.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.upper1.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 4)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 4))
			mask |= 0x1;
		    pl->contr->socket.upper1.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.upper1.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.upper1.cells[ax][ay], 0, count);
		}
		...
		/* Darkness changed */
		if (pl->contr->socket.upper1.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.upper1.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
/* printf("%s\n","dddd"); */
		...
		/*	printf("%s\n",m->upper); */
char *weird = m->upper;
char *weird2 = "/usr/games/wograld/share/wograld/maps";
char weird3[70];
int vvv;
int www;
www = 0;
/* strcpy(weird3,weird2); */
for (vvv = 0; vvv < strlen(weird); vvv++)
{
	/* if(isgraph(weird[vvv])) */
	/* weird3[vvv+41] = weird[vvv]; */
	if(weird[vvv] != weird2[vvv])
	{
		weird3[www] = weird[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
{
	weird3[vvv]=NULL;
}
...
/* mu1 = retrieve_lower_map(weird3,weird);*/

mu1 = load_and_link_upper_map(m);
...
if((nx == pl->x) && (ny == pl->y))
{
	for(detector=0; detector < MAP_LAYERS; detector++)
	{
		if (GET_MAP_FACE_OBJ(mu1, nx, ny, detector))
		{
			trouble = 1;	
		}
	}
	if(trouble == 1)
	{
		printf("%s\n","view blocked above client");
		pl->contr->up1_los = 1;
		trouble = 0;
	}
	else
	{
		printf("%s\n","view clear above client");
		pl->contr->up1_los = 0;
		trouble = 0;
	}
	
}	/* only when sweep catches object */

if(pl->contr->up1_los == 1)
{
	 map_clearcell(&pl->contr->socket.upper1.cells[ax][ay],0,-1);
}
else
...
b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(mu1, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, b_layer, 2, 4))
		{
		    mask |= 0x4;
		 /*   printf("%i\n",(update_space(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, b_layer, 2, 4)));*/
		}
		if (pl->contr->socket.EMI_smooth)
		    if (update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, b_layer, 2)){
			emask |= 0x4;
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(mu1, nx, ny, o_layer))!=NULL) {
		/*	printf("%i\n",ob); */

		/*	if (ob->head) head2 = ob->head; */
    		/*	else head2 = ob; */
		/*	face = ob->face->number; */
			/*  printf("%i\n",face); */
			...
			if (((ob->arch->tail_x || ob->arch->tail_y) ||
			    (ob->head && (ob->head->arch->tail_x || ob->head->arch->tail_y))) &&
			     ob->more) {
/* printf("%s\n","drawing upper1"); */
			    face = add_head2(ob, ax, ay, 1, 4); 
				/* printf("%i\n",face);  */

			    if (!face) continue;
			} else if (!m_layer) {
/* printf("%s\n","flip m_layer and o_layer"); */
			    m_layer = o_layer;
			    m_ob = ob;
			} else if (!t_layer) {
/* printf("%s\n","flip t_layer and o_layer"); */
			    t_layer = o_layer;
			    t_ob = ob;
			} else {    /* Both layers are full */
			    /* We want to take this object if it has a higher
...
/* printf("%s\n","empty layer"); */
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, m_layer, 1, 4))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, t_layer, 0, 4))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
	...
	}   /*  if not SW and above player */
	}   /* if no vision obstruction above client */
} /*  if mu1 */
...
void draw_upper_map2(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 5))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 5))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 5))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.upper2.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask); 
		    map_clearcell(&pl->contr->socket.upper2.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.upper2.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.upper2.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 5)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 5))
			mask |= 0x1;
		    pl->contr->socket.upper2.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.upper2.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.upper2.cells[ax][ay], 0, count);
		}
		...
		/* Darkness changed */
		if (pl->contr->socket.upper2.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.upper2.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
for (vvv = 0; vvv < strlen(weird8); vvv++)
{
	
	if(weird8[vvv] != weird2[vvv])
	{
		weird9[www] = weird8[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
...
/* mu2 = retrieve_lower_map(weird9,weird8);*/

mu2 = load_and_link_upper_map(mu1);
...
if((nx == pl->x) && (ny == pl->y))
{
	for(detector=0; detector < MAP_LAYERS; detector++)
	{
		if (GET_MAP_FACE_OBJ(mu2, nx, ny, detector))
		{
			trouble = 1;	
		}
	}
...
if((pl->contr->up1_los == 1)||(pl->contr->up2_los == 1))
{
	 map_clearcell(&pl->contr->socket.upper2.cells[ax][ay],0,-1);
}
else
...
b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(mu2, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, b_layer, 2, 5))
		{
		    mask |= 0x4;
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(mu2, nx, ny, o_layer))!=NULL) {
		/*	printf("%i\n",ob); */
...
/* printf("%s\n","empty layer"); */
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, m_layer, 1, 5))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, t_layer, 0, 5))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
	...
	}   /* if no vision obstruction two floors above player */
	}   /*  if mu2 */
}
...
void draw_upper_map3(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
    ...
    uint16  mask,emask, ewhatstart,ewhatflag;
    uint8 eentrysize, extendedinfos;
    mapstruct *m, *mu1, *mu2, *mu3;
    object *m_ob, *t_ob, *ob, *head2;

  //  sl.buf=malloc(MAXSOCKSENDBUF);
   
	strcpy((char*)sl.buf,"upper3 ");
   ...
   //     esl.buf=malloc(MAXSOCKSENDBUF);
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 6))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 6))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 6))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.upper3.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask); 
		    map_clearcell(&pl->contr->socket.upper3.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.upper3.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.upper3.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 6)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 6))
			mask |= 0x1;
		    pl->contr->socket.upper3.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.upper3.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.upper3.cells[ax][ay], 0, count);
		}
		...
		/* Darkness changed */
		if (pl->contr->socket.upper3.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.upper3.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
for (vvv = 0; vvv < strlen(weirdb); vvv++)
{
	
	if(weirdb[vvv] != weird2[vvv])
	{
		weirdc[www] = weirdb[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
...
/* mu3 = retrieve_lower_map(weirdc,weirdb);*/

mu3 = load_and_link_upper_map(mu2);
...
if((nx == pl->x) && (ny == pl->y))
{
	for(detector=0; detector < MAP_LAYERS; detector++)
	{
		if (GET_MAP_FACE_OBJ(mu3, nx, ny, detector))
		{
			trouble = 1;	
		}
	}
...
if((pl->contr->up1_los == 1)||(pl->contr->up2_los == 1)||(pl->contr->up3_los == 1)   )
{
	 map_clearcell(&pl->contr->socket.upper3.cells[ax][ay],0,-1);
}
else
...
/* if no vision obstruction three floors above player */

b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(mu3, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, b_layer, 2, 6))
		{
		    mask |= 0x4;
		
		}
		if (pl->contr->socket.EMI_smooth)
		    if (update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, b_layer, 6)){
			emask |= 0x4;
		    }
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(mu3, nx, ny, o_layer))!=NULL) {
		/*	printf("%i\n",ob); */
...
/* printf("%s\n","empty layer"); */
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, m_layer, 1, 6))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, t_layer, 0, 6))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
...
void draw_lower_map1(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 1))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 1))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 1))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.lower1.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask);
		    map_clearcell(&pl->contr->socket.lower1.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.lower1.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.lower1.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 1)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 1))
			mask |= 0x1;
		    pl->contr->socket.lower1.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.lower1.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.lower1.cells[ax][ay], 0, count);
		}
...
/* printf("%s\n",m->path); */
		...
		/* Darkness changed */
		if (pl->contr->socket.lower1.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.lower1.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
int www;
www = 0;
/* strcpy(weird5,weird2); */
for (vvv = 0; vvv < strlen(weird4); vvv++)
{
	/* if(isgraph(weird4[vvv]))*/
	/*weird5[vvv+41] = weird4[vvv];*/
	if(weird4[vvv] != weird2[vvv])
	{
		weird5[www] = weird4[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
...
/* ml1 = retrieve_lower_map(weird5,weird4); */

ml1 = load_and_link_lower_map(m);

b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(ml1, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, b_layer, 2, 1))
		    mask |= 0x4;
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(ml1, nx, ny, o_layer))!=NULL) {
			if (((ob->arch->tail_x || ob->arch->tail_y) ||
			    (ob->head && (ob->head->arch->tail_x || ob->head->arch->tail_y))) &&
			     ob->more) {

			    face = add_head2(ob, ax, ay, 1, 1);
			    if (!face) continue;
			} else if (!m_layer) {
			    m_layer = o_layer;
			    m_ob = ob;
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, m_layer, 1, 1))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, t_layer, 0, 1))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
...
void draw_lower_map2(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 2))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 2))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 2))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.lower2.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask);
		    map_clearcell(&pl->contr->socket.lower2.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.lower2.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.lower2.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 2)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 2))
			mask |= 0x1;
		    pl->contr->socket.lower2.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.lower2.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.lower2.cells[ax][ay], 0, count);
		}
...
/* printf("%s\n",m->path); */
		...
		/* Darkness changed */
		if (pl->contr->socket.lower2.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.lower2.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
if (ml1->lower)
		{
		/*	printf("%s\n",ml1->lower); */
char *weird6 = ml1->lower;
char *weird2 = "/usr/games/wograld/share/wograld/maps";
char weird7[70];
int vvv;
int www;
www = 0;
/* strcpy(weird7,weird2); */
for (vvv = 0; vvv < strlen(weird6); vvv++)
{
	/* if(isgraph(weird4[vvv]))*/
	/*weird5[vvv+41] = weird4[vvv];*/
	if(weird6[vvv] != weird2[vvv])
	{
		weird7[www] = weird6[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
...
/* ml2 = retrieve_lower_map(weird7,weird6);*/

ml2 = load_and_link_lower_map(ml1);


b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(ml2, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, b_layer, 2, 2))
		    mask |= 0x4;
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(ml2, nx, ny, o_layer))!=NULL) {
			if (((ob->arch->tail_x || ob->arch->tail_y) ||
			    (ob->head && (ob->head->arch->tail_x || ob->head->arch->tail_y))) &&
			     ob->more) {

			    face = add_head2(ob, ax, ay, 1, 2);
			    if (!face) continue;
			} else if (!m_layer) {
			    m_layer = o_layer;
			    m_ob = ob;
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, m_layer, 1, 2))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, t_layer, 0, 2))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
...
void draw_lower_map3(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int estartlen, eoldlen, b_layer, m_layer, t_layer, o_layer,n_layer, face;
    ...
    uint16  mask,emask, ewhatstart,ewhatflag;
    uint8 eentrysize, extendedinfos;
    mapstruct *m, *ml1, *ml2, *ml3;
    object *m_ob, *t_ob, *ob;

  //  sl.buf=malloc(MAXSOCKSENDBUF);
   
	strcpy((char*)sl.buf,"lower3 ");
   ...
   //     esl.buf=malloc(MAXSOCKSENDBUF);
	...
	for(x=pl->x-pl->contr->socket.mapx/2;x<max_x;x++,ax++) {
		...
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 2, 3))
		    mask |= 0x4;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 1, 3))
		    mask |= 0x2;
		if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 3))
		    mask |= 0x1;
		 ...
		 * of the map, it shouldn't have a head
		 */
		if (pl->contr->socket.lower3.cells[ax][ay].darkness != -1) {
		    SockList_AddShort(&sl, mask);
		    map_clearcell(&pl->contr->socket.lower3.cells[ax][ay],0,-1);
		}
	    } else if (d>3) {
		int need_send=0, count;
		/* This block deals with spaces that are not visible for whatever
		 ...
		 */
		if (d==4) {
		    if (pl->contr->socket.darkness && pl->contr->socket.lower3.cells[ax][ay].darkness != d) {
			mask |= 8;
			SockList_AddShort(&sl, mask);
			SockList_AddChar(&sl, 0);
		    }
		...
		{
		    SockList_AddShort(&sl, mask);
		    if (pl->contr->socket.lower3.cells[ax][ay].darkness != -1) need_send=1;
		    count = -1;
		}

		if (pl->contr->socket.mapmode == Map1aCmd && have_head(ax, ay, 3)) {
		    /* Now check to see if any heads need to be sent */
		    ...
		    if (check_head2(&sl, &pl->contr->socket, ax, ay, 0, 3))
			mask |= 0x1;
		    pl->contr->socket.lower3.cells[ax][ay].darkness = count;

		} else {
		    struct map_cell_struct *cell = &pl->contr->socket.lower3.cells[ax][ay];
		    /* properly clear a previously sent big face */
		    if(cell->faces[0] != 0
		    || cell->faces[1] != 0
		    || cell->faces[2] != 0)
			need_send = 1;
		    map_clearcell(&pl->contr->socket.lower3.cells[ax][ay], 0, count);
		}
...
/* printf("%s\n",m->path); */
		...
		/* Darkness changed */
		if (pl->contr->socket.lower3.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
		    pl->contr->socket.lower3.cells[ax][ay].darkness = d;
		    mask |= 0x8;    /* darkness bit */
...
for (vvv = 0; vvv < strlen(weirdd); vvv++)
{
	
	if(weirdd[vvv] != weird2[vvv])
	{
		weirde[www] = weirdd[vvv];
		www += 1;
	}
}
for(vvv=www;vvv<70;vvv++)
...
/* ml3 = retrieve_lower_map(weirde,weirdd);*/

ml3 = load_and_link_lower_map(ml2);


b_layer = 0;
		for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
		    if (GET_MAP_FACE_OBJ(ml3, nx, ny, o_layer)) {
			b_layer = o_layer;
			break;
		    }
		}
		...
		/* Floor face */
		if (update_space2(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, b_layer, 2, 3))
		    mask |= 0x4;
		...
		/* o_layer is set above */
		for (; o_layer < MAP_LAYERS; o_layer++) {
		    if ((ob=GET_MAP_FACE_OBJ(ml3, nx, ny, o_layer))!=NULL) {
			if (((ob->arch->tail_x || ob->arch->tail_y) ||
			    (ob->head && (ob->head->arch->tail_x || ob->head->arch->tail_y))) &&
			     ob->more) {

			    face = add_head2(ob, ax, ay, 1, 3);
			    if (!face) continue;
			} else if (!m_layer) {
			    m_layer = o_layer;
			    m_ob = ob;
		...
		/* Middle face */
		if (update_space2(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, m_layer, 1, 3))
		    mask |= 0x2;
		...
		/* Top face */
		    ...
		    if (update_space2(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, t_layer, 0, 3))
			mask |= 0x1;
		    if (pl->contr->socket.EMI_smooth && 
			update_smooth(&esl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0)){
			    emask |= 0x1;
...
static int map2_add_ob(int ax, int ay, int layer, object *ob, SockList *sl, 
		       socket_struct *ns, int *has_obj, int is_head, int floor)
{
    uint16  face_num;
	...
	for (l = layer-1; l<=layer+1; l++) {
	    ...
	    if (heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + l] == head) break;
	}
	/* Didn't find it.  So we need to store it away.  Try to store it
	 * on our original layer, and then move up a layer.
	 */
	if (l == (layer+2)) {
	    if (!heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + layer])
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS* (1+floor) + layer] = head;
	    else if ((layer + 1) <MAP_LAYERS && !heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS * (1+floor) + layer+1])
		heads[(by * MAX_HEAD_POS + bx) * MAP_LAYERS * (1+floor)+ layer + 1] = head;
	}
	return 0;
...
static int map2a_add_ob(int ax, int ay, int layer, object *ob, SockList *sl,
 ...
 * Map2 protocol
  */
static void check_space_for_heads(int ax, int ay, SockList *sl, socket_struct *ns, int floor)
{
	...
	if (heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor)+ layer]) {
	    ...
	    got_one += map2_add_ob(ax, ay, layer, 
		       heads[(ay * MAX_HEAD_POS + ax) * MAP_LAYERS * (1+floor)+ layer],
		       sl, ns, &has_obj, 1, floor);
	} else {
...
void draw_client_map2(object *pl)
{
	...
	for(x=(pl->x - pl->contr->socket.mapx / 2); x< max_x ; x++,ax++) {
	     ...
	     */
	    if (ax >= pl->contr->socket.mapx || ay >= pl->contr->socket.mapy) {
		check_space_for_heads(ax, ay, &sl, &pl->contr->socket, 0);
		...
		if (!m) {
			...
			if (ob)  {
			    g1 = has_obj;
			    got_one += map2_add_ob(ax, ay, layer, ob, &sl, &pl->contr->socket, &has_obj, 0, 0);
			 ...
			 * shows up.
			 */
	// not sure why it is adequate to send a clear byte, rather than manually clear faces
	// because clearing on the java wograld client largely affects fog of war, and seems to leave data as it is
	// although the java crossfire client was programmed to paint a solid square
			sl.len = oldlen +2;		/* 2 bytes for coordinate */
			SockList_AddChar(&sl, 0);	/* Clear byte */
			SockList_AddChar(&sl, 255);	/* Termination byte */
			map_clearcell(&pl->contr->socket.lastmap.cells[ax][ay],0,0);
...
void draw_client_map2a(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen, layer;
    sint16 nx, ny;
    SockList sl;
...
int b_layer, m_layer, t_layer, o_layer,n_layer;
 b_layer = 0;
                for (o_layer=0; o_layer < MAP_LAYERS; o_layer++) {
                 // in map1a was //  if (GET_MAP_FACE_OBJ(m, nx, ny, o_layer)) {
		  ob1 = GET_MAP_FACE_OBJ(m,nx,ny,o_layer);
		  if(ob1){
                        b_layer = o_layer;
                        break;
                    }
                }
// if nothing is found, b_layer will be left at 0, but this is also true if there is something on layer 0?
// update_space may be called with a map layer that is empty, as when b_layer defaults to 0 (nothing found) rather
// than being set to 0 (something found at map layer 0)
// instead of update_space, which might or might not add an x,y,l statement to packet, use what is normally in map2
// update_space also had the role of storing data in heads as needed
// map2_add_ob does this also, if is_head is set to 0, as it is here normally in map2 when x,y is in view of socket
// update_space also had the role of updating lastmap.cells as needed
// map2_add_ob does this also
// update space searches all map layers for more info, if certain heads data is found, for stuffing p_layer in question
// this is outside scope of map2_add_ob, although when is_head is set to 1, it has been called with ob from heads[]
// and combined with the caller, effectively writes heads[] back into lastmap.cells and the packet

// I see now that the original map2 protocol added layers in order from 0 to 9 as needed, but that map1a adds them from 2 down to 0
// may need to switch this around?

		    if(ob1){
		      g1 = has_obj;
		      // might be nothing found
		      got_one += map2_add_ob(ax,ay,2,ob1,&sl,&pl->contr->socket,&has_obj,0,0);
		      // has_obj can be incremented, but still return 0, if face is unchanged
		      // but do not increment has_obj if storing head instead
...
// as before 
                            if (g1 == has_obj)
                                del_one += map2_delete_layer(ax, ay, 2, &sl, &pl->contr->socket);
	// was // del_one += map2_delete_layer(ax, ay, layer, &sl, &pl->contr->socket);
...
// unlike regular map2 (regular, ugh) cannot rely upon maplayer loop to treat proto layers similarly
// we assume that smooth stuff was set in map2_add_ob, so leave it alone here unlike map1a proto
		m_layer=0;
                t_layer=0;
                n_layer=0;
 /* o_layer is set above */
                for (; o_layer < MAP_LAYERS; o_layer++) {
                    if ((ob2=GET_MAP_FACE_OBJ(m, nx, ny, o_layer))!=NULL) {
                        if (((ob2->arch->tail_x || ob2->arch->tail_y) ||
                            (ob2->head && (ob2->head->arch->tail_x || ob2->head->arch->tail_y))) &&
                             ob2->more) {

                   // in map1a was //         face = add_head(ob2, ax, ay, 1, 0);
	      // it seems that in update_space(), we get to a similar condition above, but inside another condition
	      // that seems to only happen if heads[x,y,p_layer] is NULL, or does not have a head->more and matches another ob at that x,y
	      // and then the map_layer argument retrieves an actual ob that map_layer
// in that case, op->more is not investigated  but head->more is reason to not bother with add_head()
	  // update_space() was being called in map1aproto below for p_layer=1
	// and is guaranteed to have an object on the corresponding map layer IF any object is found
	// but if we store the heads up here in add_head, then in update_space the heads will not be NULL, yet will match an ob on that x,y
	// then it will be cleared and put back
	// It seems that if the ob is multi-tile as is tested above, and is not at lower right (what is point of ob->more test?)
	// that we will store it here and not count it toward m_layer yet; perhaps we will not count it toward m_layer for this
	// x,y but for a later x,y or will not find other obs on other layers and so will go back and try to get it
	// just because it makes sense does not mean it is true.  So if we do find the ob is at lower right corner then
	// it is not stored in heads, but we effectively go to next o_layer anyway (?)  and if the face is 0 then we even meet the
	// condition that is checked right after leaving add_head

	// add_head() is capable of storing in multiple different layers of heads, and hopefully isnt incompatible with our mods to map2
	// map2_add_ob found later is capable of storing data in heads as needed (as we mentioned above) similar to the role of update_space that
	// would be later in map1a; unless of course we have no ob, in which in map1a we would try to read from heads and not store there,
	// and in map2 we would use map2_delete_layer and not try to store in heads
	 // but are you sure this is the right approach -- if we use add_head() to store a bunch of things in heads, then find m_layer
	// to be empty, then use map2_delete_layer rather than reading back from heads and sending this to cells/packet, then we
	// have messed up?  or is it correct to wait until the next x,y coords to read back using map2_add_ob( ...heads, is_head=1)
	// so map2_add_ob has several important cases, (whether or not the passed ob is NULL or not), and whether is_head = 1
	// well we do not use null obs with map2_add_ob, but we did use null obs with update_space
	// I suppose that setting is_head=1 does the equivalent of passing a null ob to update space, which is to use heads instead
	// but it is the caller that finds heads and passes them in, rather than update_space looking for the heads
	// also do not forget to make sure that the other references to heads and cells in this experiment, are set to use 3 p_layers

	int face = add_head(ob2, ax, ay, 1, 0);
	

      // the check mentioned before
                            if (!face) continue;
                        } else if (!m_layer) {
                            m_layer = o_layer;
                            m_ob = ob2;
...
m_ob=GET_MAP_FACE_OBJ(m, nx, ny, m_layer);
// because it m_layer may have changed in the above lines

if(m_ob){
		      g1 = has_obj;
		      // might be nothing found
		      got_one += map2_add_ob(ax,ay,1,m_ob,&sl,&pl->contr->socket,&has_obj,0,0);
		      // has_obj can be incremented, but still return 0, if face is unchanged
		      // but do not increment has_obj if storing head instead
...
// as before 
                            if (g1 == has_obj)
                                del_one += map2_delete_layer(ax, ay, 1, &sl, &pl->contr->socket);
	// was // del_one += map2_delete_layer(ax, ay, layer, &sl, &pl->contr->socket);
 ...
 /* Special logic to always send player */
	  ...
	  // need to replicate the map2 packet syntax here, but for pl->face->number
// nlayer = 0x10 + layer, layer = 0

	    int nlayer = 0x10;
            /* Length of packet */
       //     nlayer |=  len << 5, len = 2, no additions from using smooth or anim here, fireborn anim isnt used here?
      nlayer |= 2 << 5;

            SockList_AddChar(&sl, nlayer);
  SockList_AddShort(&sl, pl->face->number);
	    // already put in coordinate
	    // will put in the 0 and 255


                // in map1a was //        SockList_AddShort(&sl, pl->face->number);
                    }
                }
                /* Top face */
                else {

t_ob=GET_MAP_FACE_OBJ(m, nx, ny, m_layer);
// because it t_layer may have changed in the above lines

if(t_ob){
		      g1 = has_obj;
		      // might be nothing found
		      got_one += map2_add_ob(ax,ay,0,t_ob,&sl,&pl->contr->socket,&has_obj,0,0);
		      // has_obj can be incremented, but still return 0, if face is unchanged
		      // but do not increment has_obj if storing head instead
...
// as before 
                            if (g1 == has_obj)
                                del_one += map2_delete_layer(ax, ay, 0, &sl, &pl->contr->socket);
	// was // del_one += map2_delete_layer(ax, ay, layer, &sl, &pl->contr->socket);
                ...
                // in map1a was //    if (update_space(&sl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0, 0))
                     // not here //   mask |= 0x1;
...
// end of block modified to filter 10 layers of update_space down to 3
...
void draw_client_map2b(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
    SockList sl; 
    uint16  mask;
    mapstruct *m;
    object *m_ob, *t_ob, *ob;
...
//    sl.buf=malloc(MAXSOCKSENDBUF);
  // modify to use map2 like protocol
...
// I dont know that jxclient knows what mapextended approach to smoothing is, remove it if there are problems
...
// add this to incorporate the map scrolling of map2 proto 
if (pl->contr->socket.map_scroll_x || pl->contr->socket.map_scroll_y) {
        int coord = ((pl->contr->socket.map_scroll_x + MAP2_COORD_OFFSET) & 0x3f) << 10 |
                ((pl->contr->socket.map_scroll_y + MAP2_COORD_OFFSET) & 0x3f) << 4 | 1;
        pl->contr->socket.map_scroll_x=0;
        pl->contr->socket.map_scroll_y=0;
        SockList_AddShort(&sl, coord);
 ...
 mask = ((ax + MAP2_COORD_OFFSET) & 0x3f) << 10 |
                        ((ay + MAP2_COORD_OFFSET) & 0x3f) << 4;
...
// mask = (ax & 0x3f) << 10 | (ay & 0x3f) << 4;
             ...
             */
            if (ax >= pl->contr->socket.mapx || ay >= pl->contr->socket.mapy) {
                int i, got_one,got_more;

                oldlen = sl.len;
	  // use this to account for packet length as in map2
		got_more = 0;

                SockList_AddShort(&sl, mask);

	// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head3(&sl, &pl->contr->socket, ax, ay, 2, 0);


got_more += check_head3(&sl, &pl->contr->socket, ax, ay, 1, 0);


got_more += check_head3(&sl, &pl->contr->socket, ax, ay, 0, 0);


// refactor above to use loop 0..2 for layer

// packet length stuff as in map2
// note that socklist functions modify packet length
// we will see if we can get map2 proto functionality without anim speed
// it seems that map2_delete_layer is not needed in this case because it is just
// a special case of check_head for when face_num = 0
// and heads[] has already been verified to be 0 at that spot
// and if clearing one layer then will send 0 and layer num for that layer
// and eventually terminator,
// but if clearing them all will just send 0 and terminator

int not_blank = 0;

for (i=oldlen+2; i<sl.len; i++) {
                    if (sl.buf[i]) not_blank=1;
                }


if(!got_more) {
  sl.len = oldlen;
// if nothing to do on this space, do not even send coordinates
// if there is only one thing to send, send coords and also info for the one space

} else {
 
if(!not_blank){
/*at least 1 face, we know it's blank, only send coordinates*/
 sl.len = oldlen + 2;
SockList_AddChar(&sl, 0);     
} 
// whether or not there are nonzero faces, terminate the coord data
SockList_AddChar(&sl, 255);
}

continue;   /* don't do processing below */
            }  // x >= pl->contr->socket.mapx || ay >= pl->contr->socket.mapy
...
// added protocol for clearing square under map2 proto
SockList_AddChar(&sl, 0);     
SockList_AddChar(&sl, 255);
                }
            } else if (d>3) {
...
SockList_AddChar(&sl, 255);
		...
		// careful, the above is now suspect now that our protocol must follow map2 conventions
...
// emulate map2 proto by inserting the optional darkness info between coordinate and face infos
                /* Darkness changed */
                if (pl->contr->socket.lastmap.cells[ax][ay].darkness != d && pl->contr->socket.darkness) {
                    pl->contr->socket.lastmap.cells[ax][ay].darkness = d;
...
int got_more = 0;
 ...
 b_layer = 0;
                ...
                /* Floor face */
                got_more += update_space3(&sl, &pl->contr->socket, m, nx, ny, ax, ay, b_layer, 2, 0);
...
for (; o_layer < MAP_LAYERS; o_layer++) {
		...
		/* If the top layer isn't set it, set it to the middle
                ...
                /* Middle face */
                got_more += update_space3(&sl, &pl->contr->socket, m, nx, ny, ax, ay, m_layer, 1, 0);
...
nlayer2 |=  2 << 5;
            ...
            SockList_AddChar(&sl, nlayer2);
                ...
                /* Top face */
                    ...
                    got_more += update_space3(&sl, &pl->contr->socket, m, nx, ny, ax, ay, t_layer, 0, 0);
...
void draw_lower_map1c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
   ...
   // sl.buf=malloc(MAXSOCKSENDBUF);
    ...
    /* Init data to zero */
    // we have doubts now about if this really needs to be so big
    // and if our offsets into it are correct
    // in other words, not having unwanted collisions
    memset(heads, 0, sizeof(object *) * MAX_HEAD_POS * MAX_HEAD_POS * MAP_LAYERS * 7);
/* x,y are the real map locations.  ax, ay are viewport relative
     * locations.
     */
...
// add this to incorporate the map scrolling of map2 proto 
// except that we already did this on ground floor
// we should not need to use a placeholder, because if there were no scrolling offset
// then no coord word would be put in the packet, and nothing would need to be reset
// so it seems that if we do not reset the local map_scroll and do not send the coord
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 1);
got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 1);
got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 1);
...
SockList_AddChar(&sl, 255);
                ...
                oldlen = sl.len;
 int got_more = 0;
// not totally sure if the d==4 lines could run, and if the following lines in brackets without condition would not run
                
 SockList_AddShort(&sl, mask);
                    if (pl->contr->socket.lower1.cells[ax][ay].darkness != -1) need_send=1;
                    count = -1;

		if (have_head(ax, ay, 1)) {
                    /* Now check to see if any heads need to be sent */
                    got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 2, 1); 
                    got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 1);
                    got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 1);
...
int mflags = get_map_flags(ml1, &ml1, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(ml1, nx, ny);
                }
        if (mflags & P_NEW_MAP){
                ml1->timeout = 50;
                }
...
b_layer = 0;
              ...
              got_more += update_space4(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, b_layer, 2, 1);
...
got_more += update_space4(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, m_layer, 1, 1);


                /* Top face */
	// in the x11 multifloor protocol this has an else around it, but probably shouldnt
              got_more += update_space4(&sl, &pl->contr->socket, ml1, nx, ny, ax, ay, t_layer, 0, 1);
...
void draw_lower_map2c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
    SockList sl; /*For extended Map info*/
    uint16  mask;
    ...
    /* Init data to zero */
// unsure of this size or the offsets
    memset(heads, 0, sizeof(object *) * MAX_HEAD_POS * MAX_HEAD_POS * MAP_LAYERS * 7);
/* x,y are the real map locations.  ax, ay are viewport relative
     * locations.
     */
...
// ignore scroll coords, see lower1c
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 2);

got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 2);

got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 2);
...
int mflags = get_map_flags(ml2, &ml2, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(ml2, nx, ny);
                }
        if (mflags & P_NEW_MAP){
                ml2->timeout = 50;
                }
...
b_layer = 0;
 ...
 got_more += update_space4(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, b_layer, 2, 2);
     ...
     if (m_layer) {
                ...
                /* Middle face */
                    ...
                    got_more += update_space4(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, m_layer, 1,2); 
                ...
                /* Top face */
                       ...
                       got_more += update_space4(&sl, &pl->contr->socket, ml2, nx, ny, ax, ay, t_layer, 0, 2);
...
void draw_lower_map3c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
...
// ignore scroll data, see lower1c code for why
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 3);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 3);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 3);
...
int mflags = get_map_flags(ml3, &ml3, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(ml3, nx, ny);
                }
        if (mflags & P_NEW_MAP){
                ml3->timeout = 50;
                }
...
got_more += update_space4(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, b_layer, 2, 3);
 ...
 * visibility.  If replace what is currently
 ...
 /* If the top layer isn't set it, set it to the middle
	      ...
	      got_more += update_space4(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, m_layer, 1, 3);

                /* Top face */

                got_more += update_space4(&sl, &pl->contr->socket, ml3, nx, ny, ax, ay, t_layer, 0, 3); 
...
void draw_upper_map1c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
...
// ignore map scrolling for reasons used in lower1c
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 4);
got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 4);
got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 4);
...
if(pl->contr->up1_los == 1)
{
      struct map_cell_struct *cell2 = &pl->contr->socket.upper1.cells[ax][ay];
// if(cell2->faces[0] != 0
//                    || cell2->faces[1] != 0
//                    || cell2->faces[2] != 0){
/*
...
// if((nx < pl->x) && (ny > pl->y))
if(( ax >= (pl->contr->socket.mapx)/2 ) || (ay <= (pl->contr->socket.mapy)/2 ))
{
...
int mflags = get_map_flags(mu1, &mu1, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(mu1, nx, ny);
		}
	if (mflags & P_NEW_MAP){
                mu1->timeout = 50;
		}
...
b_layer = 0;
 ...
 got_more += update_space4(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, b_layer, 2, 4);
   ...
   /* this is a layer that doesn't have any face -
                ...
                /* Middle face */
  got_more += update_space4(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, m_layer, 1, 4);
 ...
 /* Top face */
                     ...
                     got_more += update_space4(&sl, &pl->contr->socket, mu1, nx, ny, ax, ay, t_layer, 0, 4);   
...
void draw_upper_map2c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
...
// ignore mapscroll, see lower1c for why
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 5);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 5);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 5);
...
if((pl->contr->up1_los == 1)||(pl->contr->up2_los == 1)){
//      struct map_cell_struct *cell2 = &pl->contr->socket.upper2.cells[ax][ay];
// if(cell2->faces[0] != 0
//                    || cell2->faces[1] != 0
//                    || cell2->faces[2] != 0){
...
int mflags = get_map_flags(mu2, &mu2, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(mu2, nx, ny);
                }
        if (mflags & P_NEW_MAP){
                mu2->timeout = 50;
                }
...
b_layer = 0;
                ...
                /* Floor face */
                 ...
                 got_more += update_space4(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, b_layer, 2, 5);
  ...
  if (!t_layer) {
                ...
                /* Middle face */
                   ...
                   got_more += update_space4(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, m_layer, 1, 5);
 ...
 got_more += update_space4(&sl, &pl->contr->socket, mu2, nx, ny, ax, ay, t_layer, 0, 5);
...
void draw_upper_map3c(object *pl)
{
    int x,y,ax, ay, d, startlen, max_x, max_y, oldlen;
    sint16 nx, ny;
    int b_layer, m_layer, t_layer, o_layer,n_layer, face;
...
// do not or 0x4 with mask regardless
      // account for if there is anything to send for this space
             got_more +=   check_head4(&sl, &pl->contr->socket, ax, ay, 2, 6);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 1, 6);


got_more += check_head4(&sl, &pl->contr->socket, ax, ay, 0, 6);
...
if((pl->contr->up1_los == 1)||(pl->contr->up2_los == 1)||(pl->contr->up3_los == 1)   )
{
//      struct map_cell_struct *cell2 = &pl->contr->socket.upper3.cells[ax][ay];
// if(cell2->faces[0] != 0
//                    || cell2->faces[1] != 0
//                    || cell2->faces[2] != 0){
...
int mflags = get_map_flags(mu3, &mu3, nx, ny, &nx, &ny);
            if (!(mflags & P_OUT_OF_MAP)){
            if (mflags & P_NEED_UPDATE){
                update_position(mu3, nx, ny);
                }
        if (mflags & P_NEW_MAP){
                mu3->timeout = 50;
                }
...
b_layer = 0;
               ...
               got_more += update_space4(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, b_layer, 2, 6);
 ...
 face = add_head2(ob, ax, ay, 1, 6);
             ...
             /* this is a layer that doesn't have any face -
                ...
                /* Middle face */
                       ...
                       got_more += update_space4(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, m_layer, 1, 6);

               /* Top face */
               
                   
                   got_more += update_space4(&sl, &pl->contr->socket, mu3, nx, ny, ax, ay, t_layer, 0, 6);
...
/*	up1 = pl->contr->socket.upper1; */
    ...
    for(j = (pl->y - pl->contr->socket.mapy/2) ; j < (pl->y + (pl->contr->socket.mapy+1)/2); j++) {
	    ...
	    ay=j;
	    m = pm;
	// attempts to get north etc tilemaps
	     ...
	     */
	/*	update_fake_upper_map(m,up1,ax,ay); */
	    if (mflags & P_NEW_MAP) 
		m->timeout = 50;
	}
    }
...
int previous_max_floor =3;

 if(pl->contr->up1_los == 1){
                previous_max_floor = 0;
        } else if(pl->contr->up2_los == 1){
                previous_max_floor = 1;
        } else if(pl->contr->up3_los == 1){
                previous_max_floor = 2;
        } else {
                previous_max_floor = 3;
        }

        // the map drawing code modifies the up_los variables of the pl

		draw_client_map2b(pl);
 ...
 highest_floor = 3;
        ...
        // if(highest_floor != previous_max_floor){

         send_enfl(highest_floor,&pl->contr->socket);
 // LOG(llevError,"got here\n");
...
void esrv_map_scroll(socket_struct *ns,int dx,int dy)
{
	...
	struct Map newup1map, newup2map, newup3map, newdown1map, newdown2map, newdown3map;
...
for(x=0; x<mx; x++) {
        for(y=0; y<my; y++) {
            if(x >= ns->mapx || y >= ns->mapy) {
                ...
                memset(&newdown1map.cells[x][y], 0, sizeof(struct map_cell_struct));
                memset(&newdown2map.cells[x][y], 0, sizeof(struct map_cell_struct));
                memset(&newdown3map.cells[x][y], 0, sizeof(struct map_cell_struct));
        ...
        memcpy(&(newdown1map.cells[x][y]),
           &(ns->lower1.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));

        memcpy(&(newdown2map.cells[x][y]),
           &(ns->lower2.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));

        memcpy(&(newdown3map.cells[x][y]),
           &(ns->lower3.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));
...
for(x=0; x<mx; x++) {
        for(y=0; y<my; y++) {
            if(x >= ns->mapx || y >= ns->mapy || ((x < (ns->mapx)/2) &&(y > (ns->mapy)/2))) {
                /* clear cells outside the viewable area */
                
                memset(&newup1map.cells[x][y], 0, sizeof(struct map_cell_struct));
                memset(&newup2map.cells[x][y], 0, sizeof(struct map_cell_struct));
                memset(&newup3map.cells[x][y], 0, sizeof(struct map_cell_struct));

                


            } else if ((dx < 0) && (dy == 0)) {
		if (((y+dy) >= 0) && ((y+dy) <= (ns->mapy)/2) && ((x+dx) < 0)){
			 memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));

		} else if(((y+dy) > (ns->mapy)/2) && ((y+dy) < (ns->mapy)) && ((x+dx) < (ns->mapx)/2)){
			 memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));
//			newup1map.cells[x][y].darkness = -1;
//			newup2map.cells[x][y].darkness = -1;
//			newup3map.cells[x][y].darkness = -1;

		} else {
    memcpy(&(newup1map.cells[x][y]),
           &(ns->upper1.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));

        memcpy(&(newup2map.cells[x][y]),
           &(ns->upper2.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));

        memcpy(&(newup3map.cells[x][y]),
           &(ns->upper3.cells[x+dx][y+dy]),sizeof(struct map_cell_struct));

		}

	// in double-ineqs, should cases for the after of one coordinate, be matched with before of other coord?
	    } else if ((dx < 0) && (dy < 0)) {
		if(((y+dy) >= 0) && ((y+dy) <= (ns->mapy)/2) && ((x+dx) < 0)){
			memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
...
//			newup3map.cells[x][y].darkness = -1;
		...
		}else if ( ((x+dx) >= 0) && ((x+dx) < (ns->mapx)/2) && ((y+dy) > (ns->mapy)/2)) {
			memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));
//			newup1map.cells[x][y].darkness = -1;
...
//			newup3map.cells[x][y].darkness = -1;
	    	...
	    	} else if( ((x+dx) >= (ns->mapx)/2) && ((x+dx) < (ns->mapx)) && ((y+dy) > (ns-> mapy))) {
			memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));

		} else if ((x >= (ns->mapx)/2) && (x < (ns->mapx)) && (y >= 0) && (y <= (ns->mapy)/2) &&((y +dy) >  (ns->mapy)/2) && ((x+dx) < (ns->mapx)/2)) {
		 memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                        memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                        memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));
//			newup1map.cells[x][y].darkness = -1;
...
//			newup3map.cells[x][y].darkness = -1;	
			...
			if ( ((x+dx) >= 0) && ((x+dx) < (ns->mapx)/2) && ((y+dy) > (ns->mapy)/2)) {
			memset(&(newup1map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup2map.cells[x][y]), 0, sizeof(struct map_cell_struct));
                	memset(&(newup3map.cells[x][y]), 0, sizeof(struct map_cell_struct));
	//		newup1map.cells[x][y].darkness = -1;
    ...
    memcpy(&(ns->lastmap), &newmap,sizeof(struct Map));
         memcpy(&(ns->upper1), &newup1map,sizeof(struct Map));
        memcpy(&(ns->upper2), &newup2map,sizeof(struct Map));
        memcpy(&(ns->upper3), &newup3map,sizeof(struct Map));
        memcpy(&(ns->lower1), &newdown1map,sizeof(struct Map));
        memcpy(&(ns->lower2), &newdown2map,sizeof(struct Map));
        memcpy(&(ns->lower3), &newdown3map,sizeof(struct Map));
