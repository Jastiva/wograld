...
public class DialogAttributeLong<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends DialogAttribute<G, A, R, ArchetypeAttributeLong> {
     ...
     */
    public DialogAttributeLong(@NotNull final ArchetypeAttributeLong ref, @NotNull final JFormattedTextField input) {
        super(ref);
        this.input = input;
    }
    ...
    public String getText2(@NotNull final G gameObject, @NotNull final Archetype<G, A, R> archetype, final String[] newMsg, final ArchetypeType archetypeType, final Component parent) {
        ...
        if (input.getText().trim().length() == 0) {
            if (archetype.getAttributeLong(archetypeAttributeName) != 0L) {
                return archetypeAttributeName + " 0";
            }
        } else {
            try {
                final long value = ((Number) input.getValue()).longValue();
                if (archetype.getAttributeLong(archetypeAttributeName) != value) {
