const char *rcsid_x11_png_c =
    "$Id: png.c,v 1.1.1.1 2012/09/14 03:30:02 serpentshard Exp $";
/*
...
uint8 *png_to_data(uint8 *data, int len, int *width, int *height)
{
    uint8 *pixels=NULL;
...
XPutImage(display, *pix, gc, ximage, 0, 0, 0, 0, 64, 64);
    if (has_alpha)
...
int create_and_rescale_image_from_data2(int pixmap_num, uint8 *rgba_data, int width, int height)
{
    struct PixmapInfo  *pi;
    ...
    if (pixmaps[pixmap_num+10000] != pixmaps[10000]) {
	XFreePixmap(display, pixmaps[pixmap_num+10000]->pixmap);
	if (pixmaps[pixmap_num+10000]->mask)
	    XFreePixmap(display, pixmaps[pixmap_num+10000]->mask);
	free(pixmaps[pixmap_num+10000]);
	pixmaps[pixmap_num+10000] = pixmaps[10000];
    }
