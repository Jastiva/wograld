const char *rcsid_x11_xutil_c =
    "$Id: xutil.c,v 1.1.1.1 2012/09/14 03:30:02 serpentshard Exp $";
/*
...
static KeyCode firekey[2], runkey[2], commandkey, *bind_keycode, prevkey, nextkey,
    completekey, cirtranskey;
static KeySym firekeysym[2], runkeysym[2], commandkeysym,*bind_keysym,
    prevkeysym, nextkeysym, completekeysym, cirtranskeysym;
...
void allocate_colors(Display *disp, Window w, long screen_num,
  ...
  if (vis->class >= StaticColor) {
    ...
    *colormap = DefaultColormap(disp,screen_num);
    ...
    printf("These are no longer supported by wogclnt.\n");
    return;
  }
try_private:
  if (depth > 3) {
...
void parse_keybind_line(char *buf, int line, int standard)
{
	...
	*cp1 ++ = 0;/* Null terminate it */
	...
	if (!strcmp(cpnext,"cirtranskey")) {
	    cirtranskeysym = keysym;
	    cirtranskey = keycode;
	    return;
	}
...
void init_keys(void)
{
    ...
    prevkeysym = NoSymbol;
    prevkey = 0;
    cirtranskeysym = XK_grave;
    cirtranskey = XKeysymToKeycode(display, XK_grave);
     ...
     * users home directory, "~/.wograld/keys".  Using a directory
    ...
    sprintf(buf,"%s/.wograld/keys", getenv("HOME"));
    if ((fp=fopen(buf,"r"))==NULL) {
	LOG(LOG_INFO,"x11::init_keys","Could not open ~/.wograld/keys, trying to load global bindings");
...
void parse_key_release(KeyCode kc, KeySym ks) {
	...
	stop_fire();
	else if (kc == cirtranskey || ks == cirtranskeysym)
	{
		ctrans = 0;
	}
}
...
void parse_key(char key, KeyCode keycode, KeySym keysym, int repeated)
{
	...
	if (keycode == cirtranskey || keysym == cirtranskeysym)
	{
		ctrans = 1;
	}
...
static void show_keys(int allbindings)
{
 ...
 sprintf(buf, "Circle-of-transparency Toggle Key %s (%d)", 
	  cirtranskeysym==NoSymbol?"unknown":XKeysymToString(cirtranskeysym),
	  cirtranskey);
  draw_info(buf,NDI_BLACK);
...
void bind_key(const char *params)
{
 ...
 if (!strcmp(params, "cirtranskey")) {
    bind_keycode = &cirtranskey;
    bind_keysym = &cirtranskeysym;
    draw_info("Push key to bind circle-of-transparency toggle key.",NDI_BLACK);
    cpl.input_state = Configure_Keys;
    return;
  }
...
static void save_keys(void)
{
 ...
 if (cirtranskeysym != XK_grave && cirtranskeysym != NoSymbol) {
	fprintf(fp, "! cirtranskey %s %d\n",
		XKeysymToString(cirtranskeysym), cirtranskey);
    }
