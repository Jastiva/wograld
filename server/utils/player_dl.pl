...
# Where the wograld directory is located.
$wograld_home = "${prefix}/var/wograld/players";
...
if ($playername) { # Make sure that the user typed in a playername.
	if ((open PLAYERFILE, "$wograld_home/$playername/$playername.pl") # Make sure the player's file exists
	or (open PLAYERFILE, "$wograld_home/$playername/$playername.pl.dead")) { # Or use the dead file, if no player is alive
		foreach (<PLAYERFILE>) {
			chomp; chomp;
...
if ($valid) {
	...
	if ($valid and $delete_player) {
		@files= glob("$wograld_home/$playername/*");
