 ...
 *   "$Id: skill_util.c,v 1.9 2013/02/04 23:24:41 serpentshard Exp $";
 */
/*
    CrossFire, A Multiplayer game for X-windows

    Copryight (C) 2006 Mark Wedel & Wograld Development Team
...
int do_skill (object *op, object *part, object *skill, int dir, const char *string) {
	...
	case SK_BARGAINING:
	    ...
	    break;
	case SK_MINING: 
		exp = success = do_mining(op,dir,skill);
		exp = 100*exp;
		break; 
	case SK_LUMBERJACKING:
		exp = success = do_lumberjacking(op,dir,skill);
		exp = 100*exp;
		break;
	case SK_HARVESTING:
	exp = success = do_fishing(op,dir,skill);
	exp = 100*exp;
		break;
	case SK_SORCERY:
	case SK_EVOCATION:
...
sint16 tx,ty;
    ...
    int mflags;
object *tmp = NULL;
// skill points to the instance of the mining skill that belongs to the player
...
if(skill->type == SKILL){
skill = find_skill_by_name(pl,skill->name);
/* hack to force readying of skill tool if it is available */
}
if(skill == NULL){
new_draw_info(NDI_UNIQUE,0,pl,"You did not bring your pickaxe");
return 0;
}
...
if (!tmp) {
            new_draw_info(NDI_UNIQUE, 0,pl,"There is nothing to mine here!");
        return 0;
    }

/* calculate chance of success */
/* if needed, generate object from other_ob and insert in map */
/* dont add to skill exp here, it is gotten through return of these methods */
/* and appended to pl mining_skill via the caller */

return do_skill_mining(pl,tmp,1,skill,tx,ty);

}



/* add, based on trap_disarm*/
int do_skill_mining(object *player, object *vein, int risk, object *skill,int coordx, int coordy) {
    int trapworth;  
object *ore;
/* may need to use skill -> skill , because apply_special applies a skill tool */
/* in the player->chosen_skill, which is probably here as skill */
 	
    /* this formula awards a more reasonable amount of exp */
    trapworth =  MAX(1,vein->level)  * player->map->difficulty 
         /skill->level;

    if(!(random_roll(0, (MAX(2, MIN(20,vein->level-skill->level
           +5 - player->stats.Str/2))-1), player, PREFER_LOW)))
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You successfully mine the %s!",vein->name);
          //  destroy_object(vein);
// insert the reward
// borrowed from explode_bullet
		
		if(vein->other_arch != NULL){
		ore = arch_to_object(vein->other_arch);
		insert_ob_in_map_at(ore, vein->map, vein, 0,coordx,coordy);
 remove_ob (vein);
        free_object (vein);
...
LOG (llevError, "BUG: do_skill_mining(): vein without other_arch\n");
		remove_ob (vein);
        free_object (vein);
        ...
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You fail to mine the %s",vein->name);
            if(! (random_roll(0, (MAX(2,skill->level-vein->level
               + player->stats.Dex/2-6))-1, player, PREFER_LOW)) &&risk) {
                new_draw_info(NDI_UNIQUE, 0,player,"In fact, nothing valuable can be gotten from it!");
             //   spring_trap(trap,disarmer);
		
// ensure reward is also gone
// perhaps, trigger a falling rocks trap
if(vein->other_arch != NULL){
//		ore = arch_to_object(vein->other_arch);
//		remove_ob(ore);
//		free_object(ore);
...
/* add, based on skill_attack */
int do_lumberjacking (object *pl, int dir, object *skill) {
sint16 tx,ty;
    mapstruct *m;
    int mflags;
object *tmp = NULL;
// skill points to the instance of the lumberjacking skill that belongs to the player
...
if(skill == NULL){
new_draw_info(NDI_UNIQUE,0,pl,"You did not bring your treecutting axe");
return 0;
}
...
if (!tmp) {
            new_draw_info(NDI_UNIQUE, 0,pl,"There is no tree to cut here!");
        return 0;
    }
...
/* dont add to skill exp here, it is gotten through return of these methods */
/* and appended to pl mining_skill via the caller */
return do_skill_lumberjacking(pl,tmp,1,skill,tx,ty);
// perhaps we should test that it increments a particular player's skill exp for lumberjacking
// and not all players' skill exps for lumberjack


}

/* add, based on trap_disarm*/
int do_skill_lumberjacking(object *player, object *lumber_tree, int risk, object *skill,int coordx,int coordy) {
    int trapworth;  
object *logs;
archetype *at;
object *stump;
...
/* may need to use skill -> skill , because apply_special applies a skill tool */
/* in the player->chosen_skill, which is probably here as skill */
/* if(skill->type == SKILL_TOOL){
	mskill = skill->skill;
} else if(skill->type == SKILL){
mskill = skill;
} else {
LOG (llevError, "BUG: do_skill_mining(): improper skill setup\n");
return 0;

} */
 	
    /* this formula awards a more reasonable amount of exp */
    trapworth =  MAX(1,lumber_tree->level)  * player->map->difficulty 
         / skill->level;

    if(!(random_roll(0, (MAX(2, MIN(20,lumber_tree->level-skill->level
           +5 - player->stats.Str/2))-1), player, PREFER_LOW)))
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You successfully cut the %s!",lumber_tree->name);
          //  destroy_object(vein);
// insert the reward
// borrowed from explode_bullet
		
		if(lumber_tree->other_arch != NULL){
		logs = arch_to_object(lumber_tree->other_arch);
		insert_ob_in_map_at(logs, lumber_tree->map, lumber_tree, 0,coordx+1,coordy);
		if(lumber_tree->slaying != NULL){
	       at = find_archetype_by_object_name(lumber_tree->slaying);
		if(at != NULL) {
stump = arch_to_object(at);
		insert_ob_in_map_at(stump, lumber_tree->map, lumber_tree,0,coordx,coordy);
			}
			}
 remove_ob (lumber_tree);
        free_object (lumber_tree);




		} else {
LOG (llevError, "BUG: do_skill_lumberjacking(): lumber_tree without other_arch\n");
		remove_ob (lumber_tree);
        free_object (lumber_tree);
        ...
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You fail to cut down the %s",lumber_tree->name);
            if(! (random_roll(0, (MAX(2,skill->level-lumber_tree->level
               + player->stats.Dex/2-6))-1, player, PREFER_LOW)) &&risk) {
                new_draw_info(NDI_UNIQUE, 0,player,"In fact, nothing valuable can be gotten from it!");
             //   spring_trap(trap,disarmer);
...
/* add, based on skill_attack */
int do_fishing (object *pl, int dir, object *skill) {
sint16 tx,ty;
    mapstruct *m;
    int mflags;
object *tmp = NULL;
// skill points to the instance of the fishing skill that belongs to the player
...
if(skill == NULL){
new_draw_info(NDI_UNIQUE,0,pl,"You did not bring your fishing rod");
return 0;
}
...
if (!tmp) {
            new_draw_info(NDI_UNIQUE, 0,pl,"There are no fish to harvest here!");
        return 0;
    }
...
/* dont add to skill exp here, it is gotten through return of these methods */
/* and appended to pl mining_skill via the caller */
return do_skill_fishing(pl,tmp,1,skill,tx,ty);
// perhaps we should test that it increments a particular player's skill exp for lumberjacking
// and not all players' skill exps for lumberjack
...
/* add, based on trap_disarm*/
int do_skill_fishing(object *player, object *swimming_fish, int risk, object *skill,int coordx,int coordy) {
    int trapworth;  
object *caught_fish;
archetype *at;
object *fishless_water;
...
return 0;
    ...
    /* this formula awards a more reasonable amount of exp */
    trapworth =  MAX(1,swimming_fish->level)  * player->map->difficulty 
         / skill->level;

    if(!(random_roll(0, (MAX(2, MIN(20,swimming_fish->level-skill->level
           +5 - player->stats.Dex/2))-1), player, PREFER_LOW)))
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You successfully catch the %s!",swimming_fish->name);
          //  destroy_object(vein);
// insert the reward
 if(swimming_fish->other_arch != NULL){
                caught_fish = arch_to_object(swimming_fish->other_arch);
int i=find_free_spot(caught_fish,swimming_fish->map,swimming_fish->x,swimming_fish->y,1,9);
if(i!= -1){
insert_ob_in_map_at(caught_fish,swimming_fish->map,swimming_fish,0,swimming_fish->x+freearr_x[i],swimming_fish->y+freearr_y[i]);

	//	insert_ob_in_ob(caught_fish,player);
		// make sure the player has weight remaining?
              //  insert_ob_in_map_at(caught_fish, swimming_fish->map, swimming_fish, 0,coordx+1,coordy);
		// may need to fix free_array stuff
                if(swimming_fish->slaying != NULL){
               at = find_archetype_by_object_name(swimming_fish->slaying);
                if(at != NULL) {
fishless_water = arch_to_object(at);
                insert_ob_in_map_at(fishless_water, swimming_fish->map, swimming_fish,0,coordx,coordy);
                        }
			}
                        }
 remove_ob (swimming_fish);
        free_object (swimming_fish);
...
LOG (llevError, "BUG: do_skill_fishing(): swimming_fish without other_arch\n");
		remove_ob (swimming_fish);
        free_object (swimming_fish);
        ...
        {
            new_draw_info_format(NDI_UNIQUE, 0,player,
                    "You fail to catch the %s ",swimming_fish->name);
            if(! (random_roll(0, (MAX(2,skill->level-swimming_fish->level
               + player->stats.Dex/2-6))-1, player, PREFER_LOW)) &&risk) {
                new_draw_info(NDI_UNIQUE, 0,player,"In fact, it escaped!");
             //   spring_trap(trap,disarmer);
...
LOG (llevError, "BUG: do_skill_fishing(): swimming_fish without other_arch\n");
		remove_ob (swimming_fish);
        free_object (swimming_fish);
// yes if there is no slaying, then the water dries up
// which produces the same scenario as with the tree and vein cases
// of allowing the player to pass
// however, usually the water remains, blocking the player
