...
public class DialogAttributeSpell<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends DialogAttribute<G, A, R, ArchetypeAttributeSpell> {
     ...
     */
    public DialogAttributeSpell(@NotNull final ArchetypeAttributeSpell ref, @NotNull final JComboBox input, @NotNull final Spells<NumberSpell> numberSpells) {
        super(ref);
        this.input = input;
    ...
    public String getText2(@NotNull final G gameObject, @NotNull final Archetype<G, A, R> archetype, final String[] newMsg, final ArchetypeType archetypeType, final Component parent) {
        ...
        final int attributeValue = index == 0 ? 0 : numberSpells.getSpell(index - 1).getNumber();
