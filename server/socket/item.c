 ...
 *    "$Id: item.c,v 1.4 2015/06/01 23:49:29 serpentshard Exp $";
 */
...
void esrv_draw_bankbox(object *pl)
{
    object *tmp;
    int got_one=0;
    SockList sl;
 ...
 Write_String_To_Socket(&pl->contr->socket, "delinv 0", strlen("delinv 0"));
    ...
    SockList_AddInt(&sl, 0);
for (tmp=pl->bank; tmp!=NULL; tmp=tmp->below) {
        object *head;
 if (tmp->head) head = tmp->head;
            else head = tmp;
...
static object *esrv_get_ob_from_count(object *pl, tag_t count)
{
   ...
   for(op = pl->bank; op!=NULL; op=op->below) {
	if(op->count == count) 
		return op;
    }
...
void esrv_move_object (object *pl, tag_t to, tag_t tag, long nrof)
{
     ...
     * transport.
     */
    if (!to && !pl->contr->transport) { /* drop it to the ground or ob->bank */
	// remembering that the client has been instructed that ob->bank is the ground */
...
/*          LOG(llevDebug,"Dropping object to ground that is already on ground\n");*/
	...
	// if in the bank, the item has an env of pl
if(pl->container){
	if(pl->container->type == CLOSE_BANK) {


		// drop items into bank
		// knowing that this should only happen while applying a BANKBOX
		// after the else is the basic case
		// but before the else is the active container case explained soon

		if (op->inv && QUERY_FLAG(op, FLAG_APPLIED)) {
			object *current, *next;
			for(current = op->inv; current != NULL; current=next) {
				next = current->below;
				move_item_to_bankbox(pl, pl->bank, current, 0);
			}
			esrv_update_item(UPD_WEIGHT,pl,op);
		}
		else {

			move_item_to_bankbox(pl, pl->bank,op, nrof);
			// because we did this here, there is no need for drop_object to account for bankbox
		}

	}
	// if pl->container and not pl->container->type == CLOSE_BANK
	}
	else
	{
        ...
        pl->contr->count = nrof;
        pick_up(pl, op);
	// if pick up to inventory from bankbox or from container, versus from ground,
	// probably need to handle that from pick_up
        return ;
 }
    ...
    if (pl->contr->transport) {
		...
		// if pick up to sack or to transport from bankbox or from container, versus from ground,
		// probably need to handle that from put_object_in_sack
        }
    } else {
