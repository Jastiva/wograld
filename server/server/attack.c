 ...
 *   "$Id: attack.c,v 1.4 2012/09/28 21:14:01 serpentshard Exp $";
 */
/*
...
static int hit_player_attacktype(object *op, object *hitter, int dam, 
	...
	if (op->resist[ATNR_DRAIN] >= op->resist[ATNR_LIFE_STEALING])
	  dam = (dam*(100 - op->resist[ATNR_DRAIN])) / 300;
	else dam = (dam*(100 - op->resist[ATNR_LIFE_STEALING])) / 300;
...
static int kill_object(object *op,int dam, object *hitter, int type)
{
    ...
    /* if (execute_event(op, EVENT_DEATH,hitter,NULL,NULL,SCRIPT_FIX_ALL)!=0)
                return 0; */
    /* Lauwenmark: Handle for the global event */
