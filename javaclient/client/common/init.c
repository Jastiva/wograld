const char *rcsid_common_init_c =
    "$Id: init.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $";
/*
...
void init_client_vars()
{
     ...
     */
    client_libdir=getenv("WOGCLNT_LIBDIR");
#ifdef CLIENT_LIBDIR
    ...
    want_config[CONFIG_MAPWIDTH] = 18;
    want_config[CONFIG_MAPHEIGHT] = 18;
...
#ifdef WIN32
    /* If HOME is not set, let's set it to . to avoid things like (null)/.wograld paths */
    if ( !getenv( "HOME" ) )
        {
        if ( getenv( "APPDATA" ) )
            {
