...
public abstract class AbstractArchetypeBuilder<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> {
    ...
    public void addLast(@NotNull final G invObject) {
     ...
     public void set_visibility(String vis){
         // used for prioritization of image layers on a square
         // while in server to send to client
      if (archetype == null) {
            throw new IllegalStateException();
        }
        archetype.set_visibility(vis);
         
     }
                 
     public void set_magicmap(String magicmap){
         // modifies x11 magicmap, for images of arch, in game
     if (archetype == null) {
            throw new IllegalStateException();
        }
        archetype.set_magicmap(magicmap);
         
     }
               
     public void setForegroundColor(String colorFg ){
         // also modifies x11 magicmap, for images of arch, in game
         // but write to collected as is
         if (archetype == null) {
            throw new IllegalStateException();
        }
        archetype.setForegroundColor(colorFg);
     }
         
     
     
     public void setBackgroundColor(String colorBg){
         // well, my understanding is that nothing uses this anymore
         // but write to collected as is
          if (archetype == null) {
            throw new IllegalStateException();
        }
        archetype.setBackgroundColor(colorBg);
     }
     
     public void setFloorStr(String isFl){
         // when on an arch file, modifies x11 magicmap for images of arch
        ...
        archetype.setFloorStr(isFl);
         
     }
     
     public void setQuad(String quadrant){
         // feature for wograld java client, when used on server
         // in order to not have to redraw some regions for graphics
         // that do not use it
          if (archetype == null) {
            throw new IllegalStateException();
        }
        archetype.setQuad(quadrant);
         
     }
     
     // reset in order to read next arc file
        // of course many of these were never populated this arc
        // but do not clear until all relevant faces are affected
 /*   private void clearTmpFaceData(){
        visibility_str="";
        magicmap_str="";
        color_fg_str="";
        color_bg_str="";
