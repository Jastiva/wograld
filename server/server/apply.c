...
int esrv_apply_container (object *op, object *sack)
{
	...
	if(op->container->type != CLOSE_BANK){
...
// is caused by opening or readying a non-bankbox container
		object *tmp2 = op->container->owner;
		if(tmp2 != NULL){
	...
	CLEAR_FLAG(op->container, FLAG_APPLIED);
	new_draw_info_format(NDI_UNIQUE, 0, op, "You close your bankbox");
	op->container->owner = NULL;
	op->container=NULL;
	esrv_update_item(UPD_FLAGS,op,op->bank);
...
int esrv_apply_bankbox (object *op, object *box)
{
    object *tmp=op->container;
    if(op->type!=PLAYER)
        return 0; /* This might change */

    if (box==NULL || box->type != BANKBOX) {
        LOG (llevError,
             "esrv_apply_bankbox: %s is not bankbox!\n",box?box->name:"NULL");
        return 0;
    }
     ...
     * openening the new container.
     */
// the container could be pointing to an existing sack in inventory, container on the ground,
// or be an entrypoint to bankbox

    if (op->container){
        if ((op->container->env != op) && (op->container->type != CLOSE_BANK)) { /* if container is on the ground */
            op->container->move_off = 0;

		// by applying a bankbox object,
		// not by manual or walk_off close of a container 
	   
	  // if it is a bankbox, move_off will be a property of the bankbox
	 // not of anything the container points to
        }

	if(op->container->type == CLOSE_BANK) {
		// closing by moving off bankbox
		// versus by manual apply of close_bank
		// <versus by manual_apply of a bankbox on ground>

		// if closed by moving off, is not combined
		// with opening another bankbox or container this call

		// locate the BANKBOX at the feet of the player ob
		// it doesnt matter if they walked off or were pushed
		
		// op->bank should point to the CLOSE_BANK
		// CLOSE_BANK->env should be op
		// CLOSE_BANK->below should be items or NULL
		// CLOSE_BANK->inv should be NULL
		// CLOSE_BANK->above should be NULL
		// CLOSE_BANK->owner should be the BANKBOX
		// CLOSE_BANK->map should be NULL, and all below items
		// CLOSE_BANK FLAG_REMOVED should be false, and all below items
	...
	// it may be necessary to modify esrv_apply_container
	// to account for selecting a bankbox while a container is applied

	// at this codepoint, either the container being closed
	// or the CLOSE_BANK is on op->container
	// and if a sack was open that is held by the player,
	// it must be closed
...
new_draw_info_format(NDI_UNIQUE, 0, op, "You close your bankbox");
	...
	op->contr->socket.update_look = 1;
        ...
        if (tmp == op->bank) return 1;
  }


   

    /* By the time we get here, we have made sure any other container has been closed
     */

    /* assume that it is on the ground, and that the player is
     * opening it, since if it was being closed, that would have been taken care of above.
     */
	...
	if(box->env == NULL){
        /* set these so when the player walks off, we can unapply the sack */
        box->move_off = MOVE_ALL;      /* trying force closing it */

      //  CLEAR_FLAG (sack, FLAG_APPLIED);
        new_draw_info_format(NDI_UNIQUE, 0, op, "You open your bankbox");
      //  SET_FLAG (sack, FLAG_APPLIED);
        op->container = op->bank;
	op->container->owner = box;
//	op->bank->env = 0;
	esrv_send_item(op,op->bank);
//	op->bank->env = op;
        // esrv_update_item (UPD_FLAGS, op, op->bank);
	CLEAR_FLAG(op->bank,FLAG_APPLIED);
	SET_FLAG(op->bank,FLAG_APPLIED);
	esrv_update_item(UPD_FLAGS,op,op->container);
	esrv_draw_bankbox(op);
//	op->contr->socket.update_look = 1;
        // esrv_send_inventory (op, op->bank);

    } else { /* BANKBOX is in player inventory or someone else inventory */
       new_draw_info_format(NDI_UNIQUE, 0, op, "You can't open the bankbox");
            return 0;
    }
    return 1;
}
...
void move_apply (object *trap, object *victim, object *originator)
{
   ...
   */
	...
	case BANKBOX:
	    if(victim->type == PLAYER)
		(void) esrv_apply_bankbox(victim, trap);
	    goto leave;
...
object *is_object_in_bankbox(object *pl, object *tmp){
object *tmp2, *next;
 for(tmp2 = pl->bank; tmp2 != NULL; tmp2=next){
                                                next = tmp2->below;
                                                if(next != NULL){
                                                        if(next->count == tmp->count){
                                                               return next;
                                                        }
                                                }
					}
...
int manual_apply (object *op, object *tmp, int aflag)
{
	...
	case CLOSE_BANK:
	    if(op->type==PLAYER){
		if(tmp->owner != NULL){
		(void) esrv_apply_bankbox(op,tmp->owner);
		}
		}
		return 1;
	case BANKBOX:
		if(op->type == PLAYER)
		  (void) esrv_apply_bankbox(op,tmp);
		return 1;
...
if(who->type == PLAYER){
	if(is_object_in_bankbox(who, op)){
