...
static XtResource resources[] = {
    ...
    {"cmdWograld", "CmdWograld", XtRString, sizeof (XtRString),
    Offset (cmdWograld), XtRString, NULL},
...
/* Wogedit seems to have so many memory problems, this seems like
 * a really good idea.
 */
  ...
  malloc_debug(DEBUG_MALLOC_LEVEL);
    ...
    /*XtAppInitialize (&app_con, "Wogedit", NULL, 0,  &argc, argv, 
    ...
    app = AppCreate(app_con, "Wogedit", resources, XtNumber (resources),
		    options,XtNumber(options), &argc, argv);


    /*** usage ***/
    if (argc > 1) {
      fprintf(stderr,"Usage: wogedit [-options]\n\t[-options]:\tXt-options\n\n");
      exit(2);
    }
