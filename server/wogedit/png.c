 ...
 *   "$Id: png.c,v 1.2 2011/08/16 00:07:22 serpentshard Exp $";
 */
/*
 ...
 * wograld - in addition, imLib actually has bugs which prevents it from
		   ...
		   int num, Colormap *cmap)
{
    ...
    png_get_IHDR(png_ptr, info_ptr, &localw, &localh, &bit_depth,
...
/* for now, only deal with bigfaces that are represented in parts */

if(num < 10000) {
pixmaps[num].width = localw;
pixmaps[num].height = localh;
}
  else {
pixmaps[num - 10000].width2 = localw;
pixmaps[num - 10000].height2 = localh;
 }	
     ...
     */
    if (pixels_byte==0) {
	pixels = (char*)malloc(localw * localh * bpp);
	pixels_byte =localw * localh * bpp;
    } else if ((localw * localh * bpp) > pixels_byte) {
	pixels=realloc(pixels, localw * localh * bpp);
	pixels_byte = localw * localh * bpp;
    }
    ...
    if (rows_byte == 0) {
	rows =(png_bytepp) malloc(sizeof(char*) * localh);
	rows_byte=localh;
    } else if (localh > rows_byte) {
	rows =(png_bytepp) realloc(rows, sizeof(char*) * localh);
	rows_byte=localh;
    }
    if (!rows) {
	pixels_byte=0;
...
if(num < 10000) {
pixmaps[num].pixmap =XCreatePixmap(display, draw, localw, localh,
                        DefaultDepth(display,  DefaultScreen(display)));
}
else {
pixmaps[num-10000].pixmap2 =XCreatePixmap(display, draw, localw, localh,
                        DefaultDepth(display,  DefaultScreen(display)));
}

if(num < 10000) {
gc = XCreateGC(display,pixmaps[num].pixmap,0,NULL);
}
else {
gc = XCreateGC(display, pixmaps[num-10000].pixmap2,0, NULL);
} 
...
if(num < 10000) {
pixmaps[num].mask = XCreatePixmap(display, draw, localw, localh, 1);
}
else {
pixmaps[num-10000].mask2 = XCreatePixmap(display, draw, localw, localh, 1);
}

if(num < 10000) {
gc_alpha = XCreateGC(display, pixmaps[num].mask, 0, NULL);
} else {
gc_alpha = XCreateGC(display, pixmaps[num - 10000].mask2, 0, NULL);
}
...
if(num < 10000) {
XFillRectangle(display, pixmaps[num].mask, gc_alpha, 0, 0, localw, localh);
} else {
XFillRectangle(display, pixmaps[num - 10000].mask2, gc_alpha, 0, 0, localw, localh);
}
...
if(num < 10000){
pixmaps[num].mask = None;
} else {
pixmaps[num - 10000].mask2 = None;
}
...
for(y = 0; y< localh; y++) {
	for(x = 0; x < localw; x++) {
		...
		if (alpha==0) {
		if(num < 10000){
		XDrawPoint(display, pixmaps[num].mask, gc_alpha, x, y);
		} else {
		XDrawPoint(display, pixmaps[num - 10000].mask2,gc_alpha, x,y);
			}
		}
	    }
	    if (need_color_alloc) {
...
if(num < 10000){
XPutImage(display, pixmaps[num].pixmap, gc, ximage, 0, 0, 0, 0, localw, localh);
} else {
XPutImage(display, pixmaps[num - 10000].pixmap2, gc, ximage, 0, 0, 0, 0, localw, localh);
}
