...
def find_player():
	...
	while obj != None:
		if obj.Type == Wograld.Type.PLAYER:
			return obj
		obj = obj.Above
	return None
...
def handle_say():
	...
	for d in Wograld.DirectionName.keys():
		if msg == Wograld.DirectionName[d].lower():
			want_dir = d
			break
...
def do_handle():
	...
	if evt.Subtype == Wograld.EventType.SAY:
		handle_say()
	elif evt.Subtype == Wograld.EventType.TIME:
		handle_move()


evt = Wograld.WhatIsEvent()
me = Wograld.WhoAmI()
pl = Wograld.WhoIsActivator()

Wograld.SetReturnValue(1)
