 ...
 *   "$Id: CrEdit.c,v 1.5 2011/08/16 00:07:22 serpentshard Exp $";
 */
...
static void UpdatePosition2(Widget w, int x, int y, Boolean inv);
void UpdatePosition3(Widget w, int x, int y, Boolean inv);
...
static void SetSize(Widget w)
{
    ...
    if(self->crEdit.map) {
	request.width = (MAP_WIDTH(self->crEdit.map) * self->crEdit.fontSize + MAP_HEIGHT(self->crEdit.map)*self->crEdit.fontSize);
	request.height = (MAP_WIDTH(self->crEdit.map) * self->crEdit.fontSize + MAP_HEIGHT(self->crEdit.map) * self->crEdit.fontSize);
    } else {
	request.width = self->crEdit.fontSize;
	request.height = self->crEdit.fontSize;
    }
    request.x = request.y = -0.5*request.width;
...
static void UpdatePosition (Widget w, int x, int y,Boolean inv)
{
    ...
    if(out_of_map(self->crEdit.map,x,y)) return; 
        New_Face *f;
if(!out_of_map(self->crEdit.map,x,y)){
 ...
 op=get_map_ob(self->crEdit.map, x, y);
            ...
            if (displaymode == Dm_Png || f->number != blank_face->number)
		{
                DrawFacePart (w, self->crEdit.gc, f,
                    (x+y)*0.5 * self->crEdit.fontSize,
    (y-x+MAP_WIDTH(self->crEdit.map))*0.5 * self->crEdit.fontSize,
             0, 0);  
		}
        } else {
...
static void UpdatePosition2 (Widget w, int x, int y,Boolean inv)
{
    ...
    if(out_of_map(self->crEdit.map,x-1,y+1)) return;
        New_Face *f;
if(!out_of_map(self->crEdit.map,x-1,y+1)){
if (GET_MAP_FLAGS(self->crEdit.map, x-1, y+1) & P_NEED_UPDATE) {
           /* update_position(self->crEdit.map, x-1 y+1); */
        }
 op=get_map_ob(self->crEdit.map, x-1, y+1);
        while(op != NULL) {
        if (op == NULL) {
            ; /* Paint nothing */
        } else if (QUERY_FLAG(op, FLAG_TEAR_DOWN) && self->crEdit.show_weak_walls) {
            ...
            if (displaymode == Dm_Png || f->number != blank_face->number)
                {
                DrawFacePart3(w, self->crEdit.gc, f, (x+y)*0.5*self->crEdit.fontSize,
        (y-x+MAP_WIDTH(self->crEdit.map))*0.5*self->crEdit.fontSize,0,0);
                }
        } else {
            if (displaymode == Dm_Png || op->face->number != blank_face->number)
                {
                DrawPartObject3( w, self->crEdit.gc,op,(x+y)*0.5*self->crEdit.fontSize,
                ( y-x+MAP_WIDTH(self->crEdit.map))*0.5*self->crEdit.fontSize);
                }
         }
        if(op->above != NULL)
		      ...
		      String * argv, Cardinal * argc)
{
    ...
    screen_x = event->xbutton.x;
    screen_y = event->xbutton.y;
    y = (screen_x + screen_y) / (self->crEdit.fontSize) - (MAP_WIDTH(self->crEdit.map))/2;
    x = (screen_x - screen_y) / (self->crEdit.fontSize) +(MAP_HEIGHT(self->crEdit.map))/3 + (MAP_HEIGHT(self->crEdit.map))/10 -10  + (MAP_WIDTH(self->crEdit.map))/12 + 9*(MAP_WIDTH(self->crEdit.map))/(MAP_HEIGHT(self->crEdit.map));
			  ...
			  String * argv, Cardinal * argc)
{
	...
	y = (event->xbutton.x + event->xbutton.y) / (self->crEdit.fontSize) - (MAP_WIDTH(self->crEdit.map))/2;
    x = (event->xbutton.x - event->xbutton.y) / (self->crEdit.fontSize) +(MAP_HEIGHT(self->crEdit.map))/3 + (MAP_HEIGHT(self->crEdit.map))/10 -10 +(MAP_WIDTH(self->crEdit.map))/12 + 9*(MAP_WIDTH(self->crEdit.map))/(MAP_HEIGHT(self->crEdit.map));
		     ...
		     String * argv, Cardinal * argc)
{
	...
	y = (event->xbutton.x + event->xbutton.y) / (self->crEdit.fontSize) - (MAP_WIDTH(self->crEdit.map))/2;
    x = (event->xbutton.x - event->xbutton.y) / (self->crEdit.fontSize)+(MAP_HEIGHT(self->crEdit.map))/3+(MAP_HEIGHT(self->crEdit.map))/10  -9 -(MAP_WIDTH(self->crEdit.map))/12 +(MAP_WIDTH(self->crEdit.map))/8 + 9*(MAP_WIDTH(self->crEdit.map))/(MAP_HEIGHT(self->crEdit.map));
...
static void Redisplay(Widget w,XEvent *event,Region region)
{
    ...
    int sample_pixel_x, sample_pixel_y, screen_coord_x, screen_coord_y,
	tile_x, tile_y;

  /*  for(sample_pixel_y=0; sample_pixel_y < rect.height - (self->crEdit.fontSize); */
/*	sample_pixel_y = sample_pixel_y + (self-> crEdit.fontSize)/2) {  */
     for(sample_pixel_y=rect.height - (self->crEdit.fontSize); sample_pixel_y > 0; 
        sample_pixel_y = sample_pixel_y - (self -> crEdit.fontSize)/2) { 
    	for(sample_pixel_x=0; sample_pixel_x < rect.width + 1;
	    sample_pixel_x = sample_pixel_x + self-> crEdit.fontSize) {
		screen_coord_x = sample_pixel_x + rect.x;
		screen_coord_y = sample_pixel_y + rect.y;
		/* smash these into the above loop later */
		tile_x = (screen_coord_x + screen_coord_y)/64
			- (MAP_WIDTH(self->crEdit.map  ))/ 2 - 12;
		tile_y = (screen_coord_x - screen_coord_y)/64
			+ (MAP_HEIGHT(self->crEdit.map  )) / 2 - 6;
			UpdatePosition(w,tile_x,tile_y,False);
	}
	sample_pixel_y = sample_pixel_y - (self-> crEdit.fontSize)/2;
	if(sample_pixel_y > 0 ) {
	for(sample_pixel_x=(self->crEdit.fontSize)/2; sample_pixel_x < rect.width + 1;
            sample_pixel_x = sample_pixel_x + self-> crEdit.fontSize) {
                screen_coord_x = sample_pixel_x + rect.x;
                screen_coord_y = sample_pixel_y + rect.y;
                /* smash these into the above loop later */
 ...
 for(sample_pixel_y=rect.height - (self->crEdit.fontSize); sample_pixel_y > 0;  
        ...
        for(sample_pixel_x=0; sample_pixel_x < rect.width + 1;
            sample_pixel_x = sample_pixel_x + self-> crEdit.fontSize) {
                ...
                tile_y = (screen_coord_x - screen_coord_y)/64
                        + (MAP_HEIGHT(self->crEdit.map  )) / 2 - 6;
                        UpdatePosition2(w,tile_x,tile_y,False);
        }
        sample_pixel_y = sample_pixel_y - (self-> crEdit.fontSize)/2;
        if(sample_pixel_y > 0 ) {
        for(sample_pixel_x=(self->crEdit.fontSize)/2; sample_pixel_x < rect.width + 1;
...
void CrEditRefresh2(Widget w,XRectangle rect)
{
    CrEditWidget self = (CrEditWidget)w;
    if(!w) return;
