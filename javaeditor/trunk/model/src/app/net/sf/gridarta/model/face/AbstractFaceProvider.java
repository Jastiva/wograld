...
public abstract class AbstractFaceProvider implements FaceProvider {
     ...
     * because the keys are definitely referenced elsewhere.
     */
    private final Map<String, SoftReference<ImageIcon>> cache2 = new HashMap<String, SoftReference<ImageIcon>>();
    ...
    public ImageIcon getSecondImageForFacename(@NotNull final String faceName) {
        ImageIcon icon = null;
        final SoftReference<ImageIcon> ref = cache2.get(faceName);
        if (ref != null) {
            icon = ref.get();
        }
        if (icon == null) {
            icon = createImage2(faceName);
            if (icon != null) {
                cache2.put(faceName, new SoftReference<ImageIcon>(icon));
            }
        }
        return icon;
    }
    ...
    protected abstract ImageIcon createImage(@NotNull String faceName);
     ...
     @Nullable
    protected abstract ImageIcon createImage2(@NotNull String faceName);
