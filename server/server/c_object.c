 ...
 */
// modified by serpentshard to add bankbox features
...
if (pl->type == PLAYER) {
	    ...
	    if(pl->type == PLAYER){
		// is container on ground or in inventory?
				if(tmp->env == pl){
					// if from bankbox, do not use remove_ob?
					object *tmp3, *next;
					for(tmp3 = pl->bank; tmp3 != NULL; tmp3=next){
						next = tmp3->below;
						if(next != NULL){
							if(next->count == tmp->count){
								if(next->below != NULL){
									tmp3->below = next->below;
									tmp->below->above = tmp3;
					...
					if(next == NULL){
						if(pl->bank->below == NULL){
						remove_ob(tmp);	

						}
						else {
						LOG(llevError, "what now\n");
					
						// item was not in bankbox, which is otherwise not empty
						 remove_ob(tmp);
						}
						
					}
					else {
					LOG(llevDebug, "taken out of bankbox\n");	
					}
				}
				else {
		...
		remove_ob(tmp);
		// monsters should not be concerned with above
	    }
        } else {
		LOG(llevError, "trying to pick up removed item\n");
		return;
	}
    }
...
// serpentshard modified to include bankbox
{
 ...
 tmp = stop_item (tmp);
	...
	if(op->container->type != CLOSE_BANK){
		...
		alt = op;
		// pick up items from bankbox to inventory
 ...
 if (alt->type==CONTAINER && QUERY_FLAG(alt, FLAG_APPLIED) &&
    ...
    if(tmp->env == alt) {
         ...
         */
// if picking up from bankbox, alt should already be op
// but if a standard container, this should move alt from op->container to op
        alt = op;
...
void move_item_to_bankbox (object *op, object *box, object *tmp, uint32 nrof)
{
    tag_t tmp_tag, tmp2_tag;
    object *tmp2, *sack2;
    char buf[MAX_BUF];

    if (box==tmp) return;      /* Can't put an object in itself */
    
    if (QUERY_FLAG(tmp,FLAG_STARTEQUIP)) {
        new_draw_info_format(NDI_UNIQUE, 0,op,
             "You cannot put the %s in your bankbox", query_name(tmp));
        return;
    }
    if (tmp->type == CONTAINER && tmp->inv) {
/* Eneq(@csd.uu.se): If the object to be dropped is a container
        ...
        sack2 = tmp;
        new_draw_info_format(NDI_UNIQUE, 0,op, "You move the items from %s into %s.",
                    query_name(tmp), query_name(box));
        for (tmp2 = tmp->inv; tmp2; tmp2 = tmp) {
            tmp = tmp2->below;
		// no check for capacity of box?
		move_item_to_bankbox(op, box, tmp2,0);
            // sack_can_hold(op, op->container, tmp2,tmp2->nrof)) 
         //  else { sprintf(buf,"Your %s fills up.", query_name(sack));
 ...
 if(QUERY_FLAG(tmp, FLAG_APPLIED)) {
    ...
    new_draw_info_format(NDI_UNIQUE, 0,op, "You put the %s in your bankbox",
                  query_name(tmp));
    tmp_tag = tmp->count;

 if(!QUERY_FLAG(tmp,FLAG_REMOVED)) {
    dump_object(tmp);
    LOG(llevError,"Trying to bankbox (ob) inserted object.\n%s\n", errmsg);
  //  return op;
	return;
  }
 // if(box==NULL) {
...
//    tmp2 = insert_ob_in_ob(tmp, sack);
	...
	if(op->bank->below != NULL){
	tmp->below = op->bank->below;
	op->bank->below->above = tmp;
	op->bank->below = tmp;
	}
	else {
	op->bank->below = tmp;
	...
	// by design, bankbox items should not contribute to weight of player

    fix_player(op); /* This is overkill, fix_player() is called somewhere */
  /* in object.c */
// need some way to update the weight of the player or container the item was taken from
// unless of course it was from the ground
// of course direct references might not care
    /* If an object merged (and thus, different object), we need to
     * delete the original.
     */
  //  if (tmp2 != tmp)
...
//        esrv_update_item (UPD_WEIGHT, op, box);
...
// command_drop only handles dropping from inv
// drop_object only handles dropping to ground
// potentially this one can drop from inv to bankbox
...
void drop(object *op, object *tmp)
{
 ...
 if (QUERY_FLAG(tmp, FLAG_INV_LOCKED)) {
	...
	if(op->container->type != CLOSE_BANK){
		...
		if(op->type == PLAYER){
			// knowing that this should only happen while applying a BANKBOX
			move_item_to_bankbox(op, op->bank, tmp, op->contr->count);
		}
	}
    } else {
        if (op->type == PLAYER)
