 ...
 *   "$Id: object.h,v 1.3 2014/12/13 22:03:00 serpentshard Exp $";
 */
...
typedef struct obj {
    ...
    struct obj	*container;	/* Current container being used.  I think this
				 ...
				 */
    struct obj  *bank;          /* for only player objects, contains the items of their bankbox
				*  which behaves as an invisible, no_drop, total weight reduction
				*  sack, but must be one per player and must be accessed only by
				*  operating objects of type BANKBOX on the floor */
    ...
    char *upway;	/* store path name to upper map for use in swapping */
    char *downway;
    ...
    uint8	will_apply;     /* See wograld.doc */
