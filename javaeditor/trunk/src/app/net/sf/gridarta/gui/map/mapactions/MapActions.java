...
public class MapActions<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> {
    ...
    /**
     * Action for "light visible".
     */
    @NotNull
    private final ToggleAction aLightVisible = (ToggleAction) ACTION_BUILDER.createToggle(true, "lightVisible", this);
    ...
    private final Action aUpperMap = ActionUtils.newAction(ACTION_BUILDER, "Map Navigation", this, "enterUpperMap");
    
    private final Action aLowerMap = ActionUtils.newAction(ACTION_BUILDER, "Map Navigation", this, "enterLowerMap");
     ...
     * @return <code>true</code> if the light is visible, or <code>false</code>
     *         if the light is invisible
     */
    @ActionMethod
    public boolean isLightVisible() {
        return doLightVisible(false, false) && mapViewSettings.isLightVisible();
    }

    /**
     * Sets whether the light of the current map should be visible.
     * @param lightVisible new visibility of light in current map,
     * <code>true</code> if the light should be visible, <code>false</code> if
     * invisible
     * @see #isLightVisible()
     */
    @ActionMethod
    public void setLightVisible(final boolean lightVisible) {
        doLightVisible(true, lightVisible);
    }
    ...
    public boolean isTileShow() {
        return /*doTileShow(false, false) && */false; // XXX: not yet implemented
    }
    ...
    @ActionMethod
    public void enterUpperMap() {
        doEnterMap(true, Direction.NORTH_EAST);
        // hopefully will not be exposed in same settings that expose northeast
    }
    
    @ActionMethod
    public void enterLowerMap() {
        doEnterMap(true, Direction.SOUTH_EAST);
        // hopefully will not be exposed in same settings that expose northeast
    }
    ...
    private void updateActions() {
        ...
        aLightVisible.setEnabled(doLightVisible(false, false));
        aLightVisible.setSelected(isLightVisible());
        ...
        aUpperMap.setEnabled(doEnterMap(false,Direction.NORTH_EAST));
        aLowerMap.setEnabled(doEnterMap(false,Direction.SOUTH_EAST));
     ...
     * @param performAction whether the action should be performed
     * @param lightVisible whether the light should be visible; ignored unless
     * <code>performAction</code> is set
     * @return whether the action was or can be performed
     * @noinspection SameReturnValue
     */
    private boolean doLightVisible(final boolean performAction, final boolean lightVisible) {
        if (performAction) {
            mapViewSettings.setLightVisible(lightVisible);
        }
