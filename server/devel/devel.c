...
/*
   Little program aimed at giving information to plugin about config of the wograld server.
...
int main (int argc, char **argv)
{
    ...
    if (argc==2){
      ...
      /*Special case, handle plugin installation dir, which is most likeley why
        user wants to use wograld-config in a configure script*/
    ...
    printf ("usage: wograld-config --Dflags");
    printf (" (gives complete Dflags line for compiler invocation)\n");
    printf ("usage: wograld-config --parameter-list");
    printf (" (show the list of available parameters)\n");
    printf ("usage: wograld-config <parameter name>");
