 ...
 *   "$Id: image.c,v 1.4 2012/11/14 21:18:48 serpentshard Exp $";
 */
...
/*
    ...
    The maintainer of this code can be reached at wograld-devel@real-time.com
*/
 ...
 */
New_Face *blank_face, *dark_faces[3], *empty_face, *smooth_face, *blocked_face;
...
static void read_face_data(void)
{
	...
	else if (!strncmp(buf,"quad",4)) {
	    on_face->quad = atoi(buf+5);
...
void read_bmap_names(void) {
	...
	if(( new_faces[i].quad < 0) || (new_faces[i].quad > 7)){
	/* try to validate weird entries for the quad value, but if symbol not present it defaults also */
		new_faces[i].quad = 7;
	}
    }
    ...
    blocked_face = &new_faces[find_face(BLOCKED_FACE_NAME,0)];
