...
struct PixmapInfo pixmaps[10000];
...
static void lookSelectCb(Widget w,XtPointer client,XtPointer call)
{
    ...
    /* CrEditRefresh2(w,self->look.rect);  */
    return;
}
...
static void lookInsertCb(Widget w,XtPointer client,XtPointer call)
{
    ...
    /* CrEditRefresh2(w,self->look.rect);  */
	/* object *op;  */
/* 	CrEditWidget s2 = (CrEditWidget)w;  */
/*	mapstruct *m;  */
/*	m=s2->crEdit.map;  */
	
/*	op = get_map_ob(s2->crEdit.map, self->look.rect.x, self->look.rect.y);  */
	 /*UpdatePosition3(w, 0, 0, False);  */
    return;
}
...
static void AppWograldCb  (Widget w, XtPointer client, XtPointer call)
{
    App self = (App)client;
    int pid;
    char *prog,*arg1;

    if(!self->res.cmdWograld) {
	CnvNotify("No command defined to *cmdWograld:","Continue",NULL);
	return;
    }
    prog = strtok(self->res.cmdWograld," \t");
	...
	if(execlp(prog,prog,arg1,NULL) == -1) {
	    char buf[BUFSIZ];
	    sprintf(buf,"cannot execute \"%s\"",self->res.cmdWograld);
	    CnvWarn(self->shell,buf);
	    exit(0);
	}
    default: /* parent */
	debug2("CbMainCrossfire() %d %s started\n",pid,self->res.cmdWograld);
	return;
    }
}
...
static void CutCb (Widget w, XtPointer client, XtPointer call)
{
    ...
    CrEditRefresh2(self->look.edit->w,self->look.rect);
    AppSelectUnset(self);
}
...
if (ReadImages(self->display, &colormap, displaymode)) {
