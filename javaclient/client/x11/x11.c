	const char *rcsid_x11_x11_c =
	    "$Id: x11.c,v 1.1.1.1 2012/09/14 03:30:02 serpentshard Exp $";
	/*
	    Wograld client, a client program for the wograld program.

	    Copyright (C) 2001-2003 Mark Wedel & Wograld Development Team
	...
	*/
	...
	/*
	 * This file handles most of the windowing stuff.  The idea is
...
/* #define GAME_WIDTH  1.5*(0.5*image_size * use_config[CONFIG_MAPWIDTH] + 5) */
 #define GAME_WIDTH  (0.5*image_size * use_config[CONFIG_MAPWIDTH] + 5) 
#define STAT_HEIGHT 140

/* Width of the inventory and look window */
/* #define INV_WIDTH   200 */
...
Window win_stats,win_stats2,win_stats3,winstat_tog,
win_message, win_message2;

Window winstat_tog2,winstat_tog3,winstat_tog4,winstat_tog5, winstat_tog6, winstat_tog7,
winstat_tog8, winstat_tog9, winstat_tog10, winstat_tog11, winstat_tog12, winstat_tog13;
...
int wininv_up=1,winlook_up=1,wininfo_up=1;
int wininv_height, winlook_height,wininv_width;
int wininv_top;
int winstat_w, winstat_h;
...
static Pixmap icon,xpm_pixmap,xpm_pixmap2,xpm_masks[XPMGCS];
static Pixmap dark1, dark2, dark3; 
static GC gc_root,gc_stats,gc_stats2,gc_stats3,gc_stattog,gc_stattog2,gc_stattog3,gc_stattog4,gc_stattog13,
        gc_stattog5, gc_stattog6, gc_stattog7, gc_stattog8, gc_stattog9, gc_stattog10, gc_stattog11, gc_stattog12,gc_message,
...
static void gen_draw_face(Drawable where,int face,int x,int y, int sx, int sy)
{
    ...
    if (pixmaps[face]->mask == None) {
      if(pixmaps[face] != pixmaps[0]){
	XCopyArea(display, pixmaps[face]->pixmap,
	    where,gc_floor,
	    sx,sy,image_size,image_size,x,y);
         }
         else {
        XCopyArea(display, pixmaps[blanknum]->pixmap, where, gc_floor,
          sx, sy, image_size, image_size, x, y);
         }
...
if (face < 20000)
{
	...
	XCopyArea(display, pixmaps[face]->pixmap,
        ...
        {
         XCopyArea(display, pixmaps[blanknum]->pixmap,
          where, gc_xpm[0], sx, sy, image_size, image_size, x, y);
        }
 }
/*
if ((face < 30000) && (face > 20000))
{
...
static void gen_draw_trans(Drawable where,Drawable input)
{
	int x,y;  

/*	Pixmap mask, mask2; */
	/* PixmapInfo tmp1; */
/*	mask = pixmaps[769]->mask; */
/*	XCopyArea(display, mask, xpm_pixmap3, gc_xpm[0], 0, 0, 64, 64, 0, 0); */
/*	mask2 = pixmaps[face] -> mask; */
/*	XCopyArea(display, mask2, xpm_pixmap3, gc_xpm[0], 0, 0, 64, 64, 0, 0); */
	/* xpm_masks[0] = xpm_pixmap3; */
/*	pixmaps[7000] = pixmaps[769]; */
/*	(pixmaps[7000] -> mask) = xpm_pixmap3; */
/*	XCopyArea(display, xpm_pixmap3, where, gc_xpm[0], 0, 0, 64, 64, 0, 0); */


/* gc2 = XCreateGC(display, pixmaps[769]->pixmap,0x000000,1); */
/*	gc2 = gc_xpm[0];  */
/*	gc2.setFunction(1);*/
/* gc2.setFunction(GXand); */
/*	XChangeGC(display, gc2, 0x8ffffe, 0x1); */
	
/*	XChangeGC(display, gc2, 0x00000000000000000000000, 0x1);  */
/*	XCopyArea(display, input, xpm_pixmap3,gc_xpm[0], 0, 0, 64, 64, 0, 0); */
/*	XCopyArea(display, pixmaps[769]->pixmap, xpm_pixmap3, gc2, 0, 0, 64, 64, 0, 0); */
/*	XCopyArea(display, xpm_pixmap3, where, gc_xpm[0], 0, 0, 64, 64, 0, 0); */

	for (y=0; y<64; y++) {  
 	for (x=0; x<64; x++) {   
	    if (!((x+y) % 3)) {     
	XCopyArea(display,input,where,gc_xpm[0],x,y,1,1,x,y); 
	    }   
	}  
    }   
}

int is_wall(int face1)
{
	if (((face1 > 5558) && (face1 < 5589)) ||
	   ((face1 > 1570) && (face1 < 1587)) ||
	   ((face1 > 240) && (face1 < 289 )) ||
	   ((face1 > 442) && (face1 < 482 )) ||
	   ((face1 > 916) && (face1 < 926 )))
	{
...
void end_windows()
{
    ...
    XFreeGC(display, gc_stats2);
    XFreeGC(display, gc_stattog);
...
gamehint.width=(GAME_WIDTH / 2);
	...
	(_Xconst char *) wograld_bits, 
	(unsigned int) wograld_width, (unsigned int)wograld_height); 
 ...
 xpm_pixmap2 = XCreatePixmap(display, def_root, image_size, image_size,
		DefaultDepth(display, DefaultScreen(display)));
...
static int get_info_display(void) {
   ...
   /* infohint.x=INV_WIDTH + WINDOW_SPACING*2;*/
    /* infohint.x=0; */
    infohint.x =INV_WIDTH + WINDOW_SPACING+150; 
   /* infohint.y=0; */
    infohint.y =GAME_WIDTH; 
    infohint.width=infodata.width=6+INFOCHARS*FONTWIDTH;
    infohint.height=infodata.height=(roothint.height/5);
    infodata.maxdisp = roothint.height/(5*FONTHEIGHT);
    /*wininv_top = infodata.height; */
    wininv_top = STAT_HEIGHT + 30;
	...
	(_Xconst char *) wograld_bits,
	(unsigned int) wograld_width, (unsigned int)wograld_height);
    XSetStandardProperties(display,infodata.win_info,"Wograld - text",
...
static int get_stats_display(void) {
   ...
   /* stathint.x=INV_WIDTH + infodata.width + WINDOW_SPACING; */
    stathint.x = INV_WIDTH + WINDOW_SPACING;
    /* stathint.y=0; */
    stathint.y = GAME_WIDTH;
    stathint.width= GAME_WIDTH;
/* stathint.width=( GAME_WIDTH / 2); */
    ...
    win_stats3 = XCreateSimpleWindow(display, win_root, stathint.x, stathint.y, 150,stathint.height+30,2,foreground,background);
    XSetWindowColormap(display, win_stats3,colormap);
    icon=XCreateBitmapFromData(display, win_stats3, (_Xconst char *) wograld_bits, (unsigned int) wograld_width, (unsigned int)wograld_height);
    XSetStandardProperties(display, win_stats3, "Wograld - status3", "crosstatus3",icon,gargv,gargc,&(stathint));
    gc_stats3 = XCreateGC(display, win_stats3,0,0);
    XSetForeground(display, gc_stats3, foreground);
    XSetBackground(display, gc_stats3, background);
    XSetFont(display, gc_stats3, font->fid);
    XSelectInput(display, win_stats3, ButtonPressMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, win_stats3);
    XSetWMProtocols(display, win_stats3, &wm_delete_window, 1); 
 
    /* win_stats2 = XCreateSimpleWindow(display, win_root, stathint.x+150,
     stathint.y, stathint.width-150, stathint.height+30, 2, foreground, background); */

  win_stats2 = XCreateSimpleWindow(display, win_root, 0,0,stathint.width - 150, stathint.height + 30, 2, foreground, background);
   winstat_w = stathint.width - 150;
   winstat_h = stathint.height +30;

    win_stats=XCreateSimpleWindow(display,win_stats2,
     0,35,stathint.width,stathint.height,2,
	foreground,background); 
    
    XSetWindowColormap(display, win_stats2, colormap);
    XSetWindowColormap(display, win_stats, colormap);
    icon=XCreateBitmapFromData(display,win_stats2, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int)wograld_height);
    XSetStandardProperties(display, win_stats2, "Wograld - status2", "crosstatus2",
    icon, gargv, gargc, &(stathint));
    icon=XCreateBitmapFromData(display,win_stats,
	(_Xconst char *) wograld_bits,
	(unsigned int) wograld_width, (unsigned int)wograld_height);
    XSetStandardProperties(display,win_stats,"Wograld - status",
	"crosstatus",icon,gargv,gargc, &(stathint));
    gc_stats2 = XCreateGC(display, win_stats2,0,0);
    gc_stats=XCreateGC(display,win_stats,0,0);
    XSetForeground(display,gc_stats2,foreground);
    XSetForeground(display,gc_stats,foreground);
    XSetBackground(display,gc_stats2,background);
    XSetBackground(display,gc_stats,background);
    XSetFont(display,gc_stats2,font->fid);
    XSetFont(display,gc_stats,font->fid);
    XSelectInput(display,win_stats2,ButtonPressMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XSelectInput(display,win_stats,ButtonPressMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, win_stats2);
    XMapRaised(display,win_stats);
    XSetWMProtocols(display,win_stats2, &wm_delete_window, 1);
    XSetWMProtocols(display, win_stats, &wm_delete_window, 1);
    winstat_tog = XCreateSimpleWindow(display, win_stats2, 0,5,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height); 
    XSetStandardProperties(display, winstat_tog, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint)); 
    gc_stattog = XCreateGC(display, winstat_tog, 0, 0);  
    XSetForeground(display, gc_stattog,foreground); 
    XSetBackground(display, gc_stattog, background);
    XSetFont(display, gc_stattog, font->fid);
    XSelectInput(display, winstat_tog,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog);
    XSetWMProtocols(display, winstat_tog, &wm_delete_window, 1);

winstat_tog13 = XCreateSimpleWindow(display, win_stats3, 0,5,20,20,2,foreground,background);
    XSetWindowColormap(display,winstat_tog13,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog13, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog13, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog13 = XCreateGC(display, winstat_tog13, 0, 0);
    XSetForeground(display, gc_stattog13,foreground);
    XSetBackground(display, gc_stattog13, background);
    XSetFont(display, gc_stattog13, font->fid);
    XSelectInput(display, winstat_tog13,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog13);
    XSetWMProtocols(display, winstat_tog13, &wm_delete_window, 1);


 winstat_tog2 = XCreateSimpleWindow(display, win_stats3, 30,5,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog2,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog2, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog2, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog2 = XCreateGC(display, winstat_tog2, 0, 0);
    XSetForeground(display, gc_stattog2,foreground);
    XSetBackground(display, gc_stattog2, background);
    XSetFont(display, gc_stattog2, font->fid);
    XSelectInput(display, winstat_tog2,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog2);
    XSetWMProtocols(display, winstat_tog2, &wm_delete_window, 1);

 winstat_tog3 = XCreateSimpleWindow(display, win_stats3, 60,5,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog3,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog3, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog3, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog3 = XCreateGC(display, winstat_tog3, 0, 0);
    XSetForeground(display, gc_stattog3,foreground);
    XSetBackground(display, gc_stattog3, background);
    XSetFont(display, gc_stattog3, font->fid);
    XSelectInput(display, winstat_tog3,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog3);
    XSetWMProtocols(display, winstat_tog3, &wm_delete_window, 1);

  winstat_tog4 = XCreateSimpleWindow(display, win_stats3, 90,5,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog4,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog4, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog4, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog4 = XCreateGC(display, winstat_tog4, 0, 0);
    XSetForeground(display, gc_stattog4,foreground);
    XSetBackground(display, gc_stattog4, background);
    XSetFont(display, gc_stattog4, font->fid);
    XSelectInput(display, winstat_tog4,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
  XMapRaised(display, winstat_tog4);
    XSetWMProtocols(display, winstat_tog4, &wm_delete_window, 1);

winstat_tog5 = XCreateSimpleWindow(display, win_stats3, 0,25,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog5,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog5, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog5, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog5 = XCreateGC(display, winstat_tog5, 0, 0);
    XSetForeground(display, gc_stattog5,foreground);
    XSetBackground(display, gc_stattog5, background);
    XSetFont(display, gc_stattog5, font->fid);
    XSelectInput(display, winstat_tog5,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog5);
    XSetWMProtocols(display, winstat_tog5, &wm_delete_window, 1);

winstat_tog6 = XCreateSimpleWindow(display, win_stats3, 30,25,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog6,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog6, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog6, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog6 = XCreateGC(display, winstat_tog6, 0, 0);
    XSetForeground(display, gc_stattog6,foreground);
    XSetBackground(display, gc_stattog6, background);
    XSetFont(display, gc_stattog6, font->fid);
    XSelectInput(display, winstat_tog6,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog6);
    XSetWMProtocols(display, winstat_tog6, &wm_delete_window, 1);

winstat_tog7 = XCreateSimpleWindow(display, win_stats3, 60,25,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog7,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog7, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog7, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog7 = XCreateGC(display, winstat_tog7, 0, 0);
    XSetForeground(display, gc_stattog7,foreground);
    XSetBackground(display, gc_stattog7, background);
    XSetFont(display, gc_stattog7, font->fid);
    XSelectInput(display, winstat_tog7,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog7);
    XSetWMProtocols(display, winstat_tog7, &wm_delete_window, 1);

winstat_tog8 = XCreateSimpleWindow(display, win_stats3, 90,25,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog8,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog8, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog8, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog8 = XCreateGC(display, winstat_tog8, 0, 0);
    XSetForeground(display, gc_stattog8,foreground);
    XSetBackground(display, gc_stattog8, background);
    XSetFont(display, gc_stattog8, font->fid);
    XSelectInput(display, winstat_tog8,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog8);
    XSetWMProtocols(display, winstat_tog8, &wm_delete_window, 1);

winstat_tog9 = XCreateSimpleWindow(display, win_stats3, 0,45,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog9,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog9, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog9, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog9 = XCreateGC(display, winstat_tog9, 0, 0);
    XSetForeground(display, gc_stattog9,foreground);
    XSetBackground(display, gc_stattog9, background);
    XSetFont(display, gc_stattog9, font->fid);
    XSelectInput(display, winstat_tog9,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
 XMapRaised(display, winstat_tog9);
    XSetWMProtocols(display, winstat_tog9, &wm_delete_window, 1);

winstat_tog10 = XCreateSimpleWindow(display, win_stats3, 30,45,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog10,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog10, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog10, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog10 = XCreateGC(display, winstat_tog10, 0, 0);
    XSetForeground(display, gc_stattog10,foreground);
    XSetBackground(display, gc_stattog10, background);
    XSetFont(display, gc_stattog10, font->fid);
    XSelectInput(display, winstat_tog10,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog10);
    XSetWMProtocols(display, winstat_tog10, &wm_delete_window, 1);

winstat_tog11 = XCreateSimpleWindow(display, win_stats3, 60,45,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog11,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog11, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog11, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog11 = XCreateGC(display, winstat_tog11, 0, 0);
    XSetForeground(display, gc_stattog11,foreground);
    XSetBackground(display, gc_stattog11, background);
    XSetFont(display, gc_stattog11, font->fid);
    XSelectInput(display, winstat_tog11,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog11);
    XSetWMProtocols(display, winstat_tog11, &wm_delete_window, 1);

winstat_tog12 = XCreateSimpleWindow(display, win_stats3, 90,45,15,15,2,foreground,background);
    XSetWindowColormap(display,winstat_tog12,colormap);
    icon=XCreateBitmapFromData(display,winstat_tog12, (_Xconst char *) wograld_bits,
    (unsigned int) wograld_width, (unsigned int) wograld_height);
    XSetStandardProperties(display, winstat_tog12, "wograld - stat_tog", "wogstat3",icon,gargv,gargc,&(stathint));
    gc_stattog12 = XCreateGC(display, winstat_tog12, 0, 0);
    XSetForeground(display, gc_stattog12,foreground);
    XSetBackground(display, gc_stattog12, background);
    XSetFont(display, gc_stattog12, font->fid);
    XSelectInput(display, winstat_tog12,ButtonPressMask|ButtonReleaseMask|KeyPressMask|KeyReleaseMask|ExposureMask);
    XMapRaised(display, winstat_tog12);
    XSetWMProtocols(display, winstat_tog12, &wm_delete_window, 1);
...
static void draw_stat_bar(int bar_pos, int height, int is_alert)
{
  if(height!=MAX_BARS_MESSAGE)  /* clear the top of the bar */
    XClearArea(display,win_stats, bar_pos+50, 30,
  ...
  XFillRectangle(display,win_stats,
         gc_stats, bar_pos+50, 30+MAX_BARS_MESSAGE-height, 10, height);
...
void draw_stats(int redraw) {
  ...
  char *s;
	...
	XDrawImageString(display,win_stats,
	    gc_stats,220,10, buff,strlen(buff));
    }
	...
	sprintf(buff,"Score: %5lld  Level: %d",cpl.stats.exp,
 	...
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,24, buff,strlen(buff));
	...
	sprintf(buff,"Hp %d/%d  Sp %d/%d Gr %d/%d",
 	...
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,38, buff,strlen(buff));
	...
	sprintf(buff,"S%2d D%2d Co%2d I%2d W%2d P%2d Ch%2d",
 	...
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,52, buff,strlen(buff));
	...
	sprintf(buff,"Wc:%3d Dam:%3d Ac:%3d Arm:%3d",
 	...
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,66, buff,strlen(buff));
  ...
  if(redraw || last_stats.speed!=cpl.stats.speed ||
	...
	if(cpl.stats.food<100 && (cpl.stats.food&4)) {
 	...
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,80, buff,strlen(buff));
	...
	strcpy(buff,cpl.range);
 	strcat(buff,"                     ");
	XDrawImageString(display,win_stats,
	    gc_stats,220,94, buff,strlen(buff));
    }
	...
	*buff = '\0';
	...
	for (i=0; i<MAX_SKILL; i++) {
	    ...
	    if ((on_skill % 2) == 1) {
		XDrawImageString(display,win_stats,gc_stats,220,
...
/*	pixmaps[2664] = dark1; */
	/*pixmaps[2664]->mask = dark1;*/
	/*pixmaps[2664]->width = 64; */
	/*pixmaps[2664]->height = 64; */
/*	pixmaps[2665] = dark2;*/
	/*pixmaps[2665]->mask = dark2;*/
	/*pixmaps[2665]->width = 64; */
	/*pixmaps[2665]->height = 64; */
/*	pixmaps[2666] = dark3;*/
	/*pixmaps[2666]->mask = dark3;*/
	/*pixmaps[2666]->width = 64; */
	/*pixmaps[2666]->height = 64;*/
		   ...
		   const char *t, const char *s)
{
    ...
    icon = XCreateBitmapFromData(display, l->win, 
			       (_Xconst char *) wograld_bits,
			       (unsigned int) wograld_width, 
			       (unsigned int)wograld_height);
...
static int get_inv_display(void)
{
   ...
   /* get_list_display ( &inv_list, 0, 0, INV_WIDTH, 
		      ...
		      "Wograld - inventory",
		      "crossinventory"); */
    get_list_display( &inv_list, 0,wininv_top,INV_WIDTH, roothint.height - wininv_top -WINDOW_SPACING,"Wograld - inventory","crossinventory");
    return 0;
}
...
static int get_look_display(void)
{
   ...
   /* get_list_display ( &look_list, 0, 
	      (2*(roothint.height - WINDOW_SPACING) / 3) + WINDOW_SPACING,
		      ...
		      "Wograld - look",
		    "crosslook"); */
    get_list_display( &look_list, 0,wininv_top,INV_WIDTH, roothint.height - wininv_top - WINDOW_SPACING, "Wograld - look", "crosslook");
    return 0;
}
...
static void resize_win_root(XEvent *event) {
   ...
   /* XMoveResizeWindow(display, infodata.win_info, inv_width + WINDOW_SPACING, 0,infodata.width,event->xconfigure.height);*/
    XMoveResizeWindow(display, infodata.win_info, inv_width + 150 + WINDOW_SPACING, (GAME_WIDTH)+WINDOW_SPACING,inv_width,infodata.height);
   /* XMoveResizeWindow(display, infodata.win_info, 0,0,inv_width,infodata.height); */
  /*  XMoveResizeWindow(display, infodata.win_info, inv_width + WINDOW_SPACING, 0, infodata.width, infodata.height); */
/* XMoveWindow(display, win_game, inv_width + infodata.width + WINDOW_SPACING, STAT_HEIGHT + WINDOW_SPACING); */
  /* XMoveWindow(display, win_game, inv_width + infodata.width + WINDOW_SPACING, 0); */
   XMoveWindow(display, win_game, inv_width + WINDOW_SPACING, 0);
  /* XMoveWindow(display, win_stats3, inv_width + infodata.width + WINDOW_SPACING, (GAME_WIDTH)+WINDOW_SPACING);*/
   XMoveWindow(display, win_stats3, inv_width + WINDOW_SPACING, (GAME_WIDTH)+WINDOW_SPACING);
   /* XMoveWindow(display, win_stats2, inv_width + infodata.width + 150+WINDOW_SPACING, (GAME_WIDTH)+WINDOW_SPACING); */
   /* XMoveWindow(display, win_stats2, inv_width + 150 + WINDOW_SPACING, (GAME_WIDTH)+WINDOW_SPACING); */
    XMoveWindow(display, win_stats2, 0, 0);

/*
    XMoveWindow(display, winstat_tog,0.5* inv_width + WINDOW_SPACING+30, 0);
    XMoveWindow(display, winstat_tog2,0.5* inv_width + WINDOW_SPACING+60, 0);
    XMoveWindow(display, winstat_tog3,0.5* inv_width + WINDOW_SPACING+90, 0);
    XMoveWindow(display, winstat_tog4,0.5* inv_width + WINDOW_SPACING+120, 0); 
*/

XMoveWindow(display, winstat_tog,30, 5);
XMoveWindow(display, winstat_tog13,30,5);
XMoveWindow(display, winstat_tog2,60, 5);
    XMoveWindow(display, winstat_tog3,90, 5);
    XMoveWindow(display, winstat_tog4,120, 5);
    XMoveWindow(display, winstat_tog5,30,25);
    XMoveWindow(display, winstat_tog6,60,25);
    XMoveWindow(display, winstat_tog7,90,25);
    XMoveWindow(display, winstat_tog8,120, 25);
    XMoveWindow(display, winstat_tog9,30, 45);
    XMoveWindow(display, winstat_tog10, 60, 45);
    XMoveWindow(display, winstat_tog11, 90, 45);
    XMoveWindow(display, winstat_tog12, 120, 45);
...
/* XResizeWindow(display, inv_list.win, inv_width,
                  (event->xconfigure.height/2) - wininv_top - WINDOW_SPACING); */
XMoveResizeWindow(display, inv_list.win, 0,wininv_top, inv_width, ((event->xconfigure.height)-wininv_top)/2 - WINDOW_SPACING);
wininv_height = ((event->xconfigure.height)-wininv_top)/2 - WINDOW_SPACING;
...
XMoveResizeWindow(display, look_list.win,0,wininv_top+((event->xconfigure.height)-wininv_top)/2,inv_width,((event->xconfigure.height)-wininv_top)/2 - WINDOW_SPACING);

winlook_height = ((event->xconfigure.height)-wininv_top)/2 - WINDOW_SPACING;
wininv_width = inv_width;
...
void check_x_events() {
    ...
    static int tog_pressed=0, tog_color=0;
    static int winstats_up=1;
	...
	if (want_config[CONFIG_CACHE] && lastupdate>5 && newimages) {
	    ...
	    draw_lists();		/* testing if this can work this way */
            gen_draw_face(winstat_tog13,2141,-32,-32,0,0); 
	}
    }
	...
	switch(event.type) {
		...
		if (cpl.input_state != Metaserver_Select) {
			...
			buttonpress_in_list(&look_list, &event.xbutton);
                       /* LOG(LOG_WARNING,"x11::check_x_events","buttonpress"); */
		    }
		    else if (event.xbutton.window==infodata.win_info) {
			buttonpress_in_info(&event.xbutton);
		    }
                    else if (event.xbutton.window==winstat_tog) {
                        tog_color = 1;
                        tog_pressed = 1;
                            if(winstats_up == 1){
                           XResizeWindow(display, win_stats2, 15, 15);
                           winstats_up = 0;
                           }
                           else
                           {
                         /*     XResizeWindow(display, win_stats2, GAME_WIDTH, STAT_HEIGHT); */
                              XResizeWindow(display, win_stats2, winstat_w, winstat_h);
                              winstats_up = 1;
                           }
                    }
                    else if(event.xbutton.window==win_stats2) {
                           if(winstats_up == 0){ 
                         /*  XResizeWindow(display, win_stats2, GAME_WIDTH,STAT_HEIGHT); */
                          XResizeWindow(display, win_stats2, winstat_w, winstat_h); 
                           winstats_up = 1;
                           }
                    else if(event.xbutton.window==win_stats) {
      /*                       XResizeWindow(display, win_stats2,GAME_WIDTH,STAT_HEIGHT); */
                              XResizeWindow(display, win_stats2, winstat_w, winstat_h);
                             winstats_up = 1;
                           }
                    ...
                    else if (event.xbutton.window==winstat_tog2) {
                        draw_info("button 2 is pressed",NDI_BLACK);
                      if(wininv_up == 1)
                      {
                         if(winlook_up == 1)
                         {
                         ...
                         {
                             /* was winlook_up == 0, wininv_up == 1 */
                              winlook_up = 1;
                              wininv_up = 0;
                         }
XMoveResizeWindow(display, look_list.win, 0, wininv_top, wininv_width, winlook_height*2);
                           XRaiseWindow(display, look_list.win);
                      }
                      else
                      {
                          winlook_up = 1;
                          wininv_up = 1;
                          XMoveResizeWindow(display, inv_list.win, 0, wininv_top, wininv_width, wininv_height);
XMoveResizeWindow(display, look_list.win, 0, wininv_top+wininv_height, wininv_width, winlook_height);


                      }
                      /*       XRaiseWindow(display, inv_list.win); */
                    }
                    else if (event.xbutton.window==winstat_tog3) {
                        draw_info("button 3 is pressed",NDI_BLACK);
                       if(winlook_up == 1) {
                          if(wininv_up = 1){
                               winlook_up = 0; 
                          }
                          else{
                               /* was winlook_up == 1, wininv_up == 0 */
                               winlook_up = 0;
                               wininv_up = 1;
                          }
                          XMoveResizeWindow(display, inv_list.win, 0, wininv_top, wininv_width, wininv_height*2);
                          XRaiseWindow(display, inv_list.win);
                       }
                       else
                       {
...
XMoveResizeWindow(display, look_list.win, 0, wininv_top+wininv_height, wininv_width, winlook_height);
                     ...
                     else if (event.xbutton.window==winstat_tog4) {
                        draw_info("button 4 is pressed", NDI_BLACK);
                        if(wininfo_up == 1){
                         wininfo_up = 0;
                         XLowerWindow(display, infodata.win_info);
                         }
                         else
                         {
                                XRaiseWindow(display, infodata.win_info);
                                wininfo_up = 1;
                         }
                    }
                    else if (event.xbutton.window==winstat_tog5) {
                        draw_info("button 5 is pressed", NDI_BLACK);
                    }
                   else if (event.xbutton.window==winstat_tog6) {
                        draw_info("button 6 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog7) {
                       draw_info("button 7 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog8) {
                       draw_info("button 8 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog9) {
                       draw_info("button 9 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog10) {
                       draw_info("button 10 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog11) {
                       draw_info("button 11 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog12) {
                      draw_info("button 12 is pressed", NDI_BLACK);
                    }
                    else if (event.xbutton.window==winstat_tog13) {
                      draw_info("button 13 is pressed", NDI_BLACK);
                    }
            ...
            case ButtonRelease:
                  if(event.xbutton.window == winstat_tog) {
                        if(tog_pressed == 1){
                        tog_pressed = 0;
                        tog_color = 0;
                        draw_info("this button is pressed", NDI_BLACK);
                        }
                  }
                  break;
 ...
 */
static void display_mapcell(int ax, int ay, int floor, int height)
{
	...
	{
		XFillRectangle(display,win_game,gc_blank,2+0.5*image_size*(ax + ay - 10),
			2+0.5*image_size*(ay - ax + 7),image_size,image_size);
		return;
    	}
...
if  ((mx + my)%2 == 1 )
    	...
    	{      
			int facenr = mapdata_face(ax, (ay - 1), layer);
    			int faceea = mapdata_face((ax + 1), ay, layer);
    		
			if ((facenr > 0) && (facenr < 10001) && (facenr != 766))
			{
	    			int w = pixmaps[facenr]->width;
	    			int h = pixmaps[facenr]->height;
                                if(pixmaps[facenr+10000] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facenr, -32, -32, (w-1)*image_size, (h-1)*image_size);
                                }
				if (the_map.cells[mx][my - 1].cleared) 
				{
	    				gen_draw_face(xpm_pixmap, dark2num, -32, -32, 0, 0);
				}
			if (use_config[CONFIG_DARKNESS] && the_map.cells[mx][my-1].have_darkness) 
				{
	    				int darkness;
	    				darkness = the_map.cells[mx][my-1].darkness;
	    				if (darkness > 192) 
					{
						gen_draw_face(xpm_pixmap, dark3num, -32, -32, 0, 0);
	    				}
					...
					{
						gen_draw_face(xpm_pixmap, dark1num, -32, -32, 0, 0);
	    				}
				} 
			}
			else
			{
				if (floor == 2){
				gen_draw_face(xpm_pixmap, blockednum, -32, -32, 0, 0);
				}
			}

			if ((faceea > 0) && (faceea < 10001) && (faceea != 766))
			{
	    			int w = pixmaps[faceea]->width;
	    			int h = pixmaps[faceea]->height;
                                if(pixmaps[faceea+10000] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, faceea, 32, -32, (w-1)*image_size, (h-1)*image_size);
                                }
				if (the_map.cells[mx +1][my].cleared) 
				{
	    				gen_draw_face(xpm_pixmap, dark2num, 32, -32, 0, 0);
				}
	    			if (use_config[CONFIG_DARKNESS] && the_map.cells[mx+1][my].have_darkness) 
				{
	    				int darkness;
	    				darkness = the_map.cells[mx+1][my].darkness;
	    				if (darkness > 192) 
					{
						gen_draw_face(xpm_pixmap, dark3num, 32, -32, 0, 0);
	    				}
			...
			if ((face > 0) && (face < 10001) && (face != 766))
			{
				...
				/*	printf("%s %i\n","upper1 draw",face);	*/
	    			...
	    			int h = pixmaps[face]->height;
                                if(pixmaps[face+10000] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, face, 0, 0, (w-1)*image_size, (h-1)*image_size); 
                                }
				if (the_map.cells[mx][my].cleared) 
				{
	    				 gen_draw_face(xpm_pixmap, dark2num, 0, 0, 0, 0); 
					/*	printf("%i\n",ax);  */
					/*	printf("%i\n",ay);  */

				}
				if (use_config[CONFIG_DARKNESS] && the_map.cells[mx][my].have_darkness) 
				{
	    				int darkness;
	    				darkness = the_map.cells[mx][my].darkness;
	    				if (darkness > 192) 
					{
						gen_draw_face(xpm_pixmap, dark3num, 0, 0, 0, 0);
	    				}
	    				else if (darkness > 128) 
					{
						gen_draw_face(xpm_pixmap, dark2num, 0, 0, 0, 0);
					...
					{
						gen_draw_face(xpm_pixmap, dark1num, 0, 0, 0, 0);
	    				}
				} 
			}
			else
			{
				if(floor == 2){
				gen_draw_face(xpm_pixmap, blockednum, 0, 0, 0, 0);
				}
			}
	
		for (layer=(3*floor + 1); layer< (3*floor + 3); layer++) 
		{  
			...
			if ((facenr > 0) && (facenr < 10001) && (facenr != 766))
			{
				...
				if ((ax < 10) && (ay > 9) && (is_wall(facenr)) && (ctrans == 1))
				{   
					int w = pixmaps[facenr]->width;
	    				int h = pixmaps[facenr]->height;
	   	 			gen_draw_face(xpm_pixmap2, facenr, -32, -32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
			...
			if ((faceea > 0) && (faceea < 10001) && (faceea != 766))
			{
				if ((ax < 9) && (ay > 8) && (is_wall(faceea)) && (ctrans == 1))
				{   
					int w = pixmaps[faceea]->width;
	    				int h = pixmaps[faceea]->height;
	   	 			gen_draw_face(xpm_pixmap2, faceea, 32, -32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
		...
		layer = 3*floor;
			...
			int facesu =  mapdata_face(ax, (ay + 1), layer);
			int facews = mapdata_face((ax - 1), ay, layer);
			if ((facesu > 0) && (facesu < 10001) && (facesu != 766))
			{
	    			int w = pixmaps[facesu]->width;
	    			int h = pixmaps[facesu]->height;
                                 if(pixmaps[facesu + 10000] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facesu, 32, 32, (w-1)*image_size, (h-1)*image_size);
                                }
				if (the_map.cells[mx][my+1].cleared) 
				{
	    				gen_draw_face(xpm_pixmap, dark2num, 32, 32, 0, 0);
			...
			if ((facews > 0) && (facews < 10001) && (facews != 766))
			{
	    			int w = pixmaps[facews]->width;
	    			int h = pixmaps[facews]->height;
                                if(pixmaps[facews+10000] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facews, -32, 32, (w-1)*image_size, (h-1)*image_size);
                                 }
				if (the_map.cells[mx-1][my].cleared) 
				{
	    				gen_draw_face(xpm_pixmap, dark2num, -32, 32, 0, 0);
	...
	/*	}    */
		...
		{ 
			...
			if ((face > 0) && (face < 10001) && (face != 766))
			{
	    			...
	    			if ((ax < 10) && (ay > 8) && (is_wall(face)) && (ctrans == 1))
				{ 
					int w = pixmaps[face]->width;
	    				int h = pixmaps[face]->height;
	   	 			gen_draw_face(xpm_pixmap2, face, 0, 0, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
		...
		{  
			...
			if ((facesu > 0)  && (facesu < 10001) && (facesu != 766))
			{
	    			if ((ax < 10) && (ay > 8) && (is_wall(facesu)) && (ctrans == 1)) 
				{
					int w = pixmaps[facesu]->width;
	    				int h = pixmaps[facesu]->height;
	   	 			gen_draw_face(xpm_pixmap2, facesu, 32, 32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
			...
			if ((facews > 0) && (facews < 10001) && (facews != 766))
			{
				if ((ax < 10) && (ay > 8) && (is_wall(facews)) && (ctrans == 1))
				{   
					int w = pixmaps[facews]->width;
	    				int h = pixmaps[facews]->height;
	   	 			gen_draw_face(xpm_pixmap2, facews, -32, 32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
		...
		layer = 3*floor;
			...
			int facenr2 = mapdata_face((ax - 1), ay, layer);
    			int faceea2 = mapdata_face(ax, (ay + 1), layer);
    		
			if ((facenr2 > 0) && (facenr2 < 10001) && (facenr2 != 766))
			{
	    			int w = pixmaps[facenr2]->width;
	    			int h = pixmaps[facenr2]->height;
                                if(pixmaps[facenr2] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facenr2 + 10000, -32, -32, (w-1)*image_size, (h-1)*image_size);
                                }
	    		
			}

			if ((faceea2 > 0) && (faceea2 < 10001) && (faceea2 != 766))
			{
	    			int w = pixmaps[faceea2]->width;
	    			int h = pixmaps[faceea2]->height;
                                if(pixmaps[faceea2] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, faceea2 + 10000, 32, -32, (w-1)*image_size, (h-1)*image_size);
                               }
	...
	/*	}   */
			...
			int face2 = mapdata_face((ax - 1), (ay + 1), layer);
			if ((face2 > 0) && (face2 < 10001) && (face2 != 766))
			{
	    			int w = pixmaps[face2]->width;
	    			int h = pixmaps[face2]->height;
                                if(pixmaps[face2] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, face2 + 10000, 0, 0, (w-1)*image_size, (h-1)*image_size);
                                }
	...
	/* draw single-tile faces first */
			...
			if ((facenr2 > 0) && (facenr2 < 10001) && (facenr2 != 766))
			{
				...
				if ((ax < 10) && (ay > 9) && (is_wall(facenr2)) && (ctrans == 1))
				{   
					int w = pixmaps[facenr2]->width;
	    				int h = pixmaps[facenr2]->height;
	   	 		gen_draw_face(xpm_pixmap2, facenr2 + 10000, -32, -32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
			...
			if ((faceea2 > 0) && (faceea2 < 10001) && (faceea2 != 766))
			{
				...
				if ((ax < 9) && (ay > 8) && (is_wall(faceea2)) && (ctrans == 1))
				{   
					int w = pixmaps[faceea2]->width;
	    				int h = pixmaps[faceea2]->height;
	   	 		gen_draw_face(xpm_pixmap2, faceea2 + 10000, 32, -32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
		...
		layer = 3*floor;
			...
			int facesu2 =  mapdata_face((ax - 1), (ay + 2), layer);
			int facews2 = mapdata_face((ax - 2), (ay + 1), layer);
			if ((facesu2 > 0) && (facesu2 < 10001) && (facesu2 != 766))
			{
	    			int w = pixmaps[facesu2]->width;
	    			int h = pixmaps[facesu2]->height;
                                    if(pixmaps[facesu2] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facesu2 + 10000, 32, 32, (w-1)*image_size, (h-1)*image_size);
                                }
	    		
			}

			if ((facews2 > 0) && (facews2 < 10001) && (facews2 != 766))
			{
	    			int w = pixmaps[facews2]->width;
	    			int h = pixmaps[facews2]->height;
                                if(pixmaps[facews2] != pixmaps[0]){
	   	 		gen_draw_face(xpm_pixmap, facews2 + 10000, -32, 32, (w-1)*image_size, (h-1)*image_size);
                                }
			}
	/*	}    */
		...
		{ 
			...
			if ((face2 > 0) && (face2 < 10001) && (face2 != 766))
			{
	    			if ((ax < 10) && (ay > 8) && (is_wall(face2)) && (ctrans == 1))
				{   
					int w = pixmaps[face2]->width;
	    				int h = pixmaps[face2]->height;
	   	 		gen_draw_face(xpm_pixmap2, face2 + 10000, 0, 0, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
		...
		{  
			...
			if ((facesu2 > 0) && (facesu2 < 10001) && (facesu2 != 766))
			{
	    			if ((ax < 10) && (ay > 8) && (is_wall(facesu2)) && (ctrans == 1))
				{   
					int w = pixmaps[facesu2]->width;
	    				int h = pixmaps[facesu2]->height;
	   	 		gen_draw_face(xpm_pixmap2, facesu2 + 10000, 32, 32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
			...
			if ((facews2 > 0) && (facews2 < 10001) && (facews2 != 766))
			{
				if ((ax < 10) && (ay > 8) && (is_wall(facews2)) && (ctrans == 1))
				{   
					int w = pixmaps[facews2]->width;
	    				int h = pixmaps[facews2]->height;
	   	 		gen_draw_face(xpm_pixmap2, facews2 + 10000, -32, 32, (w-1)*image_size, (h-1)*image_size);
					gen_draw_trans(xpm_pixmap, xpm_pixmap2);
   	 ...
   	 if (( mapdata_face(ax, ay, layer) > 0 ) && ((mx + my)%2 == 1 ))
   	 { 
	...
	/*	int sx, sy, sx2, sy2; */
		...
		/*	int facenr = mapdata_bigface(ax, (ay - 1), layer, &sx, &sy); */
    		/*	int faceea = mapdata_bigface((ax + 1), ay, layer, &sx2, &sy2);*/
    		int facenr = mapdata_bigface(ax, (ay - 1), layer); 
    		int faceea = mapdata_bigface((ax + 1), ay, layer);
			if ((facenr > 0) && (facenr < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, facenr, -32, -32, 0, 0);
				

			}
			

			if ((faceea > 0) && (faceea < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, faceea, 32, -32, 0, 0);
				
			}
			

		/*	int face = mapdata_bigface(ax, ay, layer, &sx, &sy);  */
			int face = mapdata_bigface(ax, ay, layer);
			if ((face > 0) && (face < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, face, 0, 0, 0, 0); 
		...
		/*	int facesu =  mapdata_bigface(ax, (ay + 1), layer, &sx, &sy); */
		/*	int facews = mapdata_bigface((ax - 1), ay, layer, &sx2, &sy2); */
		int facesu =  mapdata_bigface(ax, (ay + 1), layer); 
		int facews = mapdata_bigface((ax - 1), ay, layer);

			if ((facesu > 0) && (facesu < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, facesu, 32, 32, 0, 0);
				
	    		
			}
			

			if ((facews > 0) && (facews < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, facews, -32, 32, 0, 0);
		...
		/*	int facenr2 = mapdata_bigface((ax - 1), ay, layer, &sx, &sy); */
    		/*	int faceea2 = mapdata_bigface(ax, (ay + 1), layer, &sx2, &sy2); */
    		int facenr2 = mapdata_bigface((ax - 1), ay, layer); 
    		int faceea2 = mapdata_bigface(ax, (ay + 1), layer);
			if ((facenr2 > 0) && (facenr2 < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, facenr2 + 10000, -32, -32, 0, 0);
	    		
			}

			if ((faceea2 > 0) && (faceea2 < 10001))
			{
	   	 		gen_draw_face(xpm_pixmap, faceea2 + 10000, 32, -32, 0, 0);
	    		
			}
	

	
		/*	int face2 = mapdata_bigface((ax - 1), (ay + 1), layer, &sx, &sy);  */
			int face2 = mapdata_bigface((ax - 1), (ay + 1), layer);

			if ((face2 > 0) && (face2 < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, face2 + 10000, 0, 0, 0, 0);
		...
		/*	int facesu2 =  mapdata_bigface((ax - 1), (ay + 2), layer, &sx, &sy);  */
		/*	int facews2 = mapdata_bigface((ax - 2), (ay + 1), layer, &sx2, &sy2); */
			int facesu2 =  mapdata_bigface((ax - 1), (ay + 2), layer);
			int facews2 = mapdata_bigface((ax - 2), (ay + 1), layer);


			if ((facesu2 > 0) && (facesu2 < 10001))
			{
	    		
	   	 		gen_draw_face(xpm_pixmap, facesu2 + 10000, 32, 32, 0, 0);
	    		
			}

			if ((facews2 > 0) && (facews2 < 10001))
			{
	    			
	   	 		gen_draw_face(xpm_pixmap, facews2 + 10000, -32, 32, 0, 0);
			}
    	...
    	if (got_one) 
	{
		...
		/*	XSetClipMask(display, gc_xpm[XPMGCS-1], dark2_icon); */
	   	/* 	XSetClipMask(display, gc_xpm[XPMGCS-1], dark2);  */
	/* XCopyPlane(display, dark2_icon, xpm_pixmap, gc_xpm[0], 0, 0, 24, 6,0, 0, 1); */
	   	 /*	xpm_masks[XPMGCS-1] = dark2_icon; */
 /* XCopyPlane(display, dark2, xpm_pixmap, gc_xpm[0], 0, 0, 64, 64 ,0, 0, 1); */
		...
		/*	xpm_masks[XPMGCS-1] = dark2_icon; */
		/*	XCopyArea(display, dark2_icon, xpm_pixmap,gc_xpm[XPMGCS-1],0,0,image_size,image_size,0, 0); */
		/*	} */
...
void display_map_doneupdate(int redraw, int notice)
{
   ...
   for(ax=18;ax>0;ax=ax-1) {
	for(ay=1;ay<19;ay=ay+1) { 
	    mx = ax + pl_pos.x;
	    my = ay + pl_pos.y;
	    if (redraw || the_map.cells[mx][my].need_update)  
	    {
                 /*      floor 0 is ground.  floor 1 through 3 are increasingly underground */
                 /*      floor 4 through 6 are increasingly above ground */
           /*     if(floor_active[3] != 0) */
           /*     {	*/
	/* display_mapcell -3 changed to -6 to avoid segment fault */
	/*		display_mapcell(ax,ay,3, -3); */
			display_mapcell(ax,ay,3, -6);
	/*	}	*/
	/*	if(floor_active[2] != 0){  */
	/* display_mapcell -2 changed to -5 to avoid segment fault */
	/*		display_mapcell(ax,ay,2, -2);  */
			display_mapcell(ax,ay,2, -5); 
	/*	}	*/
	/*	if(floor_active[1] != 0){ */
	/*  display_mapcell -1 changed to -4 to avoid segment fault */
	/*		display_mapcell(ax,ay,1, -1);  */
			display_mapcell(ax,ay,1, -4);
	/*	} */
	
	/*  display_mapcell 0 changed to -3 to avoid segment fault */
	/*	display_mapcell(ax,ay,0,0);  */ 
		display_mapcell(ax,ay,0, -3);
		
	/*	if(floor_active[4] != 0){ */
	/*  display_mapcell 1 changed to -2 to avoid segment fault */
	/*		display_mapcell(ax,ay,4,1);  */
			display_mapcell(ax,ay,4,-2);
	/*	} */

	/*	if(floor_active[5] != 0) { */
	/*  display_mapcell 2 changed to -1 to avoid segment fault */
	/*		display_mapcell(ax,ay,5,2);   */
			display_mapcell(ax,ay,5,-1); 
	/*	}	*/
		
	/*	if(floor_active[6] != 0){	*/
	/*  display_mapcell 3 changed to 0 to avoid segment fault */
	/*		display_mapcell(ax,ay,6,3); */
			display_mapcell(ax,ay,6,0); 
	/*	}	*/
		
		if ((mx + my) %2 == 1)
		{
			XCopyArea(display,xpm_pixmap,win_game,gc_game,0,0,image_size,image_size,
	    		2+0.5*image_size*(ax + ay - 10),2+0.5*image_size*(ay - ax + 7 ));
		}
...
/*    w = use_config[CONFIG_MAPWIDTH];
 ...
 /* for(srcx=18;srcx>0;srcx=srcx-1) {
	for(srcy=1;srcy<18;srcy=srcy+1) { 
		XCopyArea(display, win_game, win_game, gc_copy,
2+0.5*image_size*(srcx + srcy - 10),2+0.5*image_size*(srcy - srcx + 7),
image_size, image_size, 2+0.5*image_size*(srcx +dx + srcy +dy - 10),2+0.5*image_size*(srcy +dy - srcx -dx + 7)); */
       
/*  srcx, srcy, */
/* 2+0.5*image_size*(w + h - 10),2+0.5*image_size*(h - w + 7), */
...
void save_winpos()
{
    ...
    sprintf(savename,"%s/.wograld/winpos", getenv("HOME"));
...
void set_window_pos()
{
    ...
    sprintf(buf,"%s/.wograld/winpos", getenv("HOME"));
...
void load_defaults()
{
    ...
    sprintf(path,"%s/.wograld/defaults", getenv("HOME"));
...
void save_defaults()
{
    ...
    fprintf(fp,"# This file is generated automatically by wogclnt.\n");
    fprintf(fp,"# Manually editing is allowed, however wogclnt may be a bit finicky about\n");
