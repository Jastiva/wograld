...
public class DialogAttributeFaceName<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends DialogAttribute<G, A, R, ArchetypeAttributeFaceName> {
     ...
     */
    public DialogAttributeFaceName(@NotNull final ArchetypeAttributeFaceName ref, @NotNull final FaceComponent input) {
        super(ref);
        this.input = input;
    }
    ...
    public String getText2(@NotNull final G gameObject, @NotNull final Archetype<G, A, R> archetype, final String[] newMsg, final ArchetypeType archetypeType, final Component parent) {
        final String inline = input.getFaceName();
    ...
    public void appendSummary(@NotNull final Document doc, @NotNull final Style style) {
        final String value = input.getFaceName();
        if (value.length() > 0) {
            addLine(doc, style, getRef().getAttributeName() + " = " + value);
        }
    }

} // DialogAttributeFaceName
