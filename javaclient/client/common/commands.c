const char *rcsid_common_commands_c =
    "$Id: commands.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $";
/*
...
#define NUM_LAYERS 2
static void map1_common(unsigned char *data, int len, int rev, int floor)
{
	 ...
	 */
	if ((mask & 0xf) == 0) {  
	 mapdata_set_face(x, y, -1, -1, -1, -1, floor);
	...
	mapdata_set_face(x, y, darkness, faces[0], faces[1], faces[2], floor);
...
{
    map1_common(data, len, 0, 0);
}
void Map1aCmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 0);
}

void Low1Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 1);
}

void Low2Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 2);
}

void Low3Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 3);
}

void Up1Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 4);
/*	printf("%i\n",99); */
}

void Up2Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 5);
}

void Up3Cmd(unsigned char *data, int len)
{
    map1_common(data, len, 1, 6);
}
...
int num_compare2( const void *a, const void *b)
{
   return ( *(int*)a - *(int*)b );
}

void FcnmCmd(unsigned char *data, int len){
short v = 0;
char namefc;
int w, notlist, emptyslot;
emptyslot = 0;
notlist = 1;
v = GetShort_String(data);
w= 0;
while(w < 5)
{
  if(important_tiles[w] == (int)v){
  notlist = 0;
  }
  if(important_tiles[w] == 0) 
  {
/* I know, this isnt robust against new entries that are zero */
/* but zero should never be used by the server, as it is a default value */
     emptyslot = w;
     break;
  }
  w++;
}
if((notlist == 1) && (emptyslot < 5))
{
   important_tiles[emptyslot] = (int)v;
   imp_count++;
}

if(imp_count == 5)
{
  qsort(important_tiles, 5, sizeof(int),num_compare);
  qsort(important_tiles, 3, sizeof(int),num_compare2);
  dark1num = important_tiles[0];
  dark2num = important_tiles[1]; 
  dark3num = important_tiles[2];
  blockednum = important_tiles[3];
  blanknum = important_tiles[4]; 
  
}



/*	namefc = GetChar_String(data); */
  /*      w = (namefc & 0xf); */
/*	if (w == 8) */
/*        if(namefc == "a") { */
...
void EnableFloorCmd(unsigned char *data, int len){

	int fl;

	fl = GetInt_String(data);
...
void DisableFloorCmd(unsigned char *data, int len){
