...
import com.realtime.wograld.jxclient.gui.gui.RendererGuiStateListener;
...
public class StatsWatcher {
     ...
     * @param server the wograld server connection to watch
     * @param soundManager the sound manager instance to watch
     */
    public StatsWatcher(@NotNull final Stats stats, @NotNull final JXCWindowRenderer windowRenderer, @NotNull final WograldServerConnection server, @NotNull final SoundManager soundManager) {
        this.soundManager = soundManager;
        poisoned = stats.getStat(WograldStatsListener.C_STAT_POISONED) != 0;
        level = stats.getStat(WograldStatsListener.CS_STAT_LEVEL);
        stats.addWograldStatsListener(statsListener);
    ...
    private void checkStats(final int statNo, final int value) {
        ...
        } else if (statNo == WograldStatsListener.CS_STAT_LEVEL) {
