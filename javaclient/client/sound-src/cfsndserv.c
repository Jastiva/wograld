static char *rcsid_cfsndserv_c =
    "$Id: cfsndserv.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $";
/*
...
#define CONFIG_FILE "/.wograld/sndconfig"
...
int init_sounds(void)
{
    ...
    sprintf(path,"%s/.wograld/sounds", getenv("HOME"));
    i=0;
    if (!(fp=fopen(path,"r"))) {
...
char *path;
  ...
  f=fopen(path,"w");
  if (!f) return -1;
  fprintf(f,"# Wograld sound server settings\n");
