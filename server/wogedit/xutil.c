 ...
 *   "$Id: xutil.c,v 1.3 2011/08/16 00:07:22 serpentshard Exp $";
 */
...
int ReadImages(Display *gdisp, Colormap *cmap, enum DisplayMode type){
...
/*    *pixmaps2 = (Pixmap *) malloc(sizeof(Pixmap *) * nrofpixmaps);  */
 /*   if (type==Dm_Png){   						*/
/*	*masks = (Pixmap *) malloc(sizeof(Pixmap *) * nrofpixmaps);   */
 /*       *masks2 = (Pixmap *) malloc(sizeof(Pixmap *) * nrofpixmaps);  */

   /*  }  */

/*	*pixm = (struct PixmapInfo *) malloc(sizeof(struct PixmapInfo *) * nrofpixmaps);  */
    ...
    if (type==Dm_Png) {
	sprintf(filename,"%s/wograld.0",settings.datadir);
#ifdef HAVE_LIBPNG
	...
	if (type==Dm_Png) {
#ifdef HAVE_LIBPNG
	    if (png_to_xpixmap(gdisp, root, databuf, len,num,cmap)) { 
	...
	if (pixmaps[i].pixmap == 0) {
	   ...
	   pixmaps[i].pixmap = pixmaps[blank_face->number].pixmap;
	}

if (type==Dm_Png) {
        sprintf(filename,"%s/wograld.2",settings.datadir);
#ifdef HAVE_LIBPNG
        /* init_pngx_loader(gdisp); */
#endif
    }
  ...
  if (fread(databuf, 1, len, infile)!=len) {
        ...
        if (type==Dm_Png) {
#ifdef HAVE_LIBPNG
            if (png_to_xpixmap(gdisp, root, databuf, len, num + 10000,cmap)){
 ...
 for (i = 0; i < nrofpixmaps; i++)
        if (pixmaps[i].pixmap2 == 0) {
            LOG(llevDebug, "Warning, pixmap %d is not defined, setting it to blank\n", i);
            pixmaps[i].pixmap2 = pixmaps[blank_face->number].pixmap2;
        }
...
void free_pixmaps(Display *gdisp)
{
    int i;

    for (i=0; i < nrofpixmaps; i++) {
	if (pixmaps[i].pixmap != 0) {
	    XFreePixmap(gdisp, pixmaps[i].pixmap);
	    pixmaps[i].pixmap = 0;
	}
    }
    ...
    for (i=0; i < nrofpixmaps; i++) {
         if(pixmaps[i].pixmap2 !=0) {
             XFreePixmap(gdisp,pixmaps[i].pixmap2);
             pixmaps[i].pixmap2=0;
         }
      }
}
