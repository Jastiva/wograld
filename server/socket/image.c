 ...
 *    "$Id: image.c,v 1.7 2015/06/01 23:49:29 serpentshard Exp $";
 */
...
void read_client_images(void)
{
	...
	sprintf(filename,"%s/wograld.%d",settings.datadir, fileno);
...
void esrv_send_face(socket_struct *ns,short face_num, int nocache)
{
    ...
    // sl.buf = (unsigned char*)malloc(MAXSOCKSENDBUF);
    // sl2.buf = (unsigned char*)malloc(MAXSOCKSENDBUF);
	sl.len=2;
	sl2.len=2;
	...
	SockList_AddShort(&sl,new_faces[face_num].quad);
	...
	if(ns->image2){
		strcpy((char*)sl2.buf, "face2 ");
		sl2.len=strlen((char*)sl2.buf);
		SockList_AddShort(&sl2,face_num+10000);
		SockList_AddChar(&sl2,2);
		SockList_AddInt(&sl2,facesets[2].faces[face_num].checksum);
		SockList_AddShort(&sl2,5);
		strcpy((char*)sl2.buf + sl2.len, new_faces[face_num].name);
		sl2.len += strlen(new_faces[face_num].name);
		Send_With_Handling(ns, &sl2);
	...
	strcpy((char*)sl2.buf, "image3 ");
	sl2.len=strlen((char*)sl2.buf);
	SockList_AddInt(&sl2, face_num);
	SockList_AddChar(&sl2, fallback);
	SockList_AddInt(&sl2, facesets[2].faces[face_num].datalen);
	memcpy(sl2.buf+sl2.len, facesets[2].faces[face_num].data, 
	       facesets[2].faces[face_num].datalen);
	sl2.len += facesets[2].faces[face_num].datalen;
