#
# Grab the wograld-images archive of the sourceforge files list.  If
...
%define _sourcedir /export/home/wograld/Client
%define _srcrpmdir /export/home/wograld/RPMS
%define _rpmdir /export/home/wograld/RPMS

Name: %{Name}-%{extra}
Version: %{version}
Release: 1
Summary: Client for connecting to wograld servers.
Group: Amusements/Games/Wograld
Copyright: GPL
URL: http://wograld.real-time.com
Source0: %{name}-%{version}.tar.gz
Source1: %{name}-sounds-%{version}.tar.gz
Source2: %{name}-images-%{version}.tar.gz
Provides: wograld-client
Requires: SDL
Requires: SDL_image
Requires: alsa-lib
...
%description
Wograld is a highly graphical role-playing adventure game with
...
Client for playing the new client/server based version of Wograld.
This package allows you to connect to wograld servers around the world.
You do not need install the wograld program in order to use this
package.

%package sounds
Summary: Sound effects for the wograld game
Group: X11/Games
Requires: wograld-client
...
%description gtk
GTK version of the wograld client
...
%build
chmod 755 configure
%configure --datadir=/usr/share/games/wograld \
	--with-sound-dir=/usr/share/sounds/wograld --disable-dmalloc
...
# Sounds
#
install -d %{buildroot}%{_datadir}/sounds/wograld
install sounds/*.raw %{buildroot}%{_datadir}/sounds/wograld
#
# Client images cd lib; adm/collect_images -archive
#
install -d %{buildroot}%{_datadir}/games/wograld/%{name}
install wograld.clsc %{buildroot}%{_datadir}/games/wograld/%{name}
install wograld.base %{buildroot}%{_datadir}/games/wograld/%{name}
install bmaps.client %{buildroot}%{_datadir}/games/wograld/%{name}
install README %{buildroot}%{_datadir}/games/wograld/%{name}
#
# KDE
#
...
# KDE
#
install -m 644 -c gtk/wograld-client.desktop \
	%{buildroot}%{_datadir}/applnk/Games/Adventure/wograld.desktop
install -m 644 pixmaps/16x16.png \
	%{buildroot}%{_datadir}/icons/hicolor/16x16/apps/wograld-client.png
install -m 644 pixmaps/32x32.png \
	%{buildroot}%{_datadir}/icons/hicolor/32x32/apps/wograld-client.png
install -m 644 pixmaps/48x48.png \
	%{buildroot}%{_datadir}/icons/hicolor/48x48/apps/wograld-client.png
install -m 644 pixmaps/16x16.png \
	%{buildroot}%{_datadir}/icons/locolor/16x16/apps/wograld-client.png
install -m 644 pixmaps/32x32.png \
	%{buildroot}%{_datadir}/icons/locolor/32x32/apps/wograld-client.png
install -m 644 pixmaps/48x48.png \
	%{buildroot}%{_datadir}/icons/locolor/48x48/apps/wograld-client.png


%post
rm -f %{_datadir}/gnome/apps/Games/wograld.desktop
rm -f %{_datadir}/gnome/ximian/Programs/Games/wograld.desktop
...
%attr(755,root,root) %{_bindir}/wogclnt
%{_mandir}/man6/wogclnt.6*

%files gtk
%defattr(644,root,root,755)
%doc CHANGES COPYING License NOTES README TODO
%attr(755,root,root) %{_bindir}/gwogclnt
%{_mandir}/man6/gwogclnt.6*
%dir %{_datadir}/games/wograld/%{name}
%attr(0444,root,root) %{_datadir}/games/wograld/%{name}/*
...
#%defattr(644,root,root,755)
#%doc CHANGES COPYING License NOTES README TODO
#%attr(755,root,root) /usr/X11R6/bin/gnome-wogclnt
#/usr/X11R6/man/man6/gnome-wogclnt.6*
#/usr/share/gnome/apps/Games/Tclug/wograld.desktop
#/usr/share/pixmaps/shield.png

%files sounds
%defattr(644,root,root,755)
%dir %{_datadir}/sounds/wograld
%attr(444,root,root) %{_datadir}/sounds/wograld/*
...
+ wograld-client-20030219CVS-1.realtime
...
* Sat Sep 28 2002 Bob Tanner <tanner@real-time.com>
  + wograld-client-1.4.0-realtime.1
  - upgrade to 1.4.0
...
* Wed Jul 25 2002 Bob Tanner <tanner@real-time.com>
  + wograld-client-1.3.1-realtime.4
  - fixed wograld-client.desktop entry
...
- x11/wogclnt.man: -font and -noautorepeat options added.
...
- Make all clients Provide: wograld-client
- Make sounds dependent on wograld-client.
...
- gnome/gnome-wogclnt.man, help/about.h, x11/wogclnt.man: Update mail address.
...
- Makefile.in: Modify so that installs the target (wogclnt, gwogclnt,
...
- CHANGES, configure configure.in wograld-client.spec: Update for 0.96.0
  release item_types item_types.h: Add some additional items.

* Sat Feb 10 2001 Bob Tanner <tanner@real-time.com>
- Created new Group for this package Amusements/Games/Wograld
...
- Moved sounds into /usr/share/sounds/wograld 
- Moved the prefix to /usr/X11R6
- Upgrade source file locations
- Made the gtk client GNOME aware and put the wograld picture into
