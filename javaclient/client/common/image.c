const char *rcsid_common_image_c =
    "$Id: image.c,v 1.1.1.1 2012/09/14 03:30:03 serpentshard Exp $";
/*
...
void init_common_cache_data()
{
 ...
 if (!use_config[CONFIG_CACHE])
    ...
    sprintf(bmaps,"%s/.wograld/wograld-images/bmaps.client", getenv("HOME"));
    if ((fp=fopen(bmaps,"r"))!=NULL) {
	while (fgets(inbuf, MAX_BUF-1, fp)!=NULL) {
...
void finish_face_cmd(int pnum, uint32 checksum, int has_sum, char *face, int faceset)
{
     ...
     * from the server.
     */
    sprintf(filename,"%s/.wograld/gfx/%s.png", getenv("HOME"), face);
	...
	if (ce->ispublic)
	    ...
	    sprintf(filename,"%s/.wograld/wograld-images/%s",
...
void Image3Cmd(uint8 *data,  int len)
{  
    int pnum,plen;
    uint8 setnum;
    ...
    plen = GetInt_String(data+5);
    if (len<9 || (len-9)!=plen) {
	LOG(LOG_WARNING,"common::Image3Cmd","Lengths don't compare (%d,%d)",
		(len-9),plen);
	return;
    }
    display_newpng2(pnum,(char*)data+9,plen, setnum);
}
...
void display_newpng(long face,uint8 *buf,long buflen, int setnum)
{
	...
	sprintf(filename, "%s/.wograld/wograld-images",getenv("HOME"));
	...
	sprintf(filename, "%s/.wograld/wograld-images/%c%c", getenv("HOME"),
	...
	do {
	    setnum++;
	    sprintf(filename, "%s/.wograld/wograld-images/%c%c/%s.%d",
	...
	if ((tmpfile = fopen(filename,"w"))==NULL) {
	     ...
	     * downloaded.
	     */
	    sprintf(filename,"%s/.wograld/wograld-images/bmaps.client", getenv("HOME"));
...
void display_newpng2(long face,uint8 *buf,long buflen, int setnum)
{
    char    filename[MAX_BUF], basename[MAX_BUF];
    uint8   *pngtmp;
    FILE *tmpfile;
    uint32 width, height, csum, i;
    Cache_Entry *ce2=NULL;
	...
	sprintf(filename, "%s/.wograld/wograld-images/%c", getenv("HOME"),
		 facetoname[face][2]);
	if (access(filename, R_OK | W_OK | X_OK)== -1) 
#ifdef WIN32
	    mkdir(filename);
	...
	do {
	    setnum++;
	    sprintf(filename, "%s/.wograld/wograld-images/%c/%s.%d",
		    getenv("HOME"), facetoname[face][2],basename, setnum);
	} while (access(filename, F_OK)==-0);
	...
	if ((tmpfile = fopen(filename,"w"))==NULL) {
	    ...
	    sprintf(filename, "%c/%s.%d", facetoname[face][2],
		    basename, setnum);
	/*    ce2 = image_add_hash(facetoname[face], filename,  csum, 0); */
	     ...
	     * downloaded.
	     */
	    sprintf(filename,"%s/.wograld/wograld-images/bmaps2.client", getenv("HOME"));
	    if ((tmpfile=fopen(filename, "a"))==NULL) {
		LOG(LOG_WARNING,"common::display_newpng2","Can not open %s for appending", filename);
	    }
	    else {
		fprintf(tmpfile, "%s %u %c/%s.%d\n",
			facetoname[face], csum, facetoname[face][2], basename, setnum);
		fclose(tmpfile);
	    }
	}
#endif
    ...
    pngtmp = png_to_data(buf, buflen, &width, &height);
    if(create_and_rescale_image_from_data2(face, pngtmp, width, height)) { 
	LOG(LOG_WARNING, "common::display_newpng", "create_and_rescale_image_from_data2 failed for face %ld", face);
    }
