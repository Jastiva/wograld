...
/**
 * A {@link net.sf.gridarta.model.io.MapArchObjectParser} for Daimonin map arch
 * object instances.
...
public class MapArchObjectParser extends AbstractMapArchObjectParser<MapArchObject> {
    ...
    public void save(@NotNull final Appendable appendable, @NotNull final MapArchObject mapArchObject) throws IOException {
        ...
        if (mapArchObject.isPermDeath()) {
            appendable.append("perm_death 1\n");
        }
        if (mapArchObject.isUltraDeath()) {
            appendable.append("ultra_death 1\n");
        }
        if (mapArchObject.isUltimateDeath()) {
        ...
        if (mapArchObject.getTilesetX() != 0) {
            format.format("tileset_x %d\n", mapArchObject.getTilesetX());
        }
        if (mapArchObject.getTilesetY() != 0) {
    ...
    protected boolean parseLine(@NotNull final String line, @NotNull final MapArchObject mapArchObject, @NotNull final BufferedReader reader) {
        ...
        if (line.startsWith("perm_death ")) {
            mapArchObject.setPermDeath(NumberUtils.parseInt(line.substring(11)) != 0);
            return true;
        }

        if (line.startsWith("ultra_death ")) {
            mapArchObject.setUltraDeath(NumberUtils.parseInt(line.substring(12)) != 0);
            return true;
        }

        if (line.startsWith("ultimate_death ")) {
            mapArchObject.setUltimateDeath(NumberUtils.parseInt(line.substring(15)) != 0);
            return true;
        }
