...
import com.realtime.wograld.jxclient.util.StringSplitter;
import org.jetbrains.annotations.NotNull;

/**
 * Describes a Wograld spell.
 * @author Lauwenmark
 */
public class Spell {
    ...
    /**
     * The spell's skill. See {@link WograldStatsListener#CS_STAT_SKILLINFO}.
     */
    private int skill;
    ...
    /**
     * Returns the spell's skill. See {@link WograldStatsListener#CS_STAT_SKILLINFO}.
    ...
    public String getTooltipText() {
        ...
        if ((path & stats.getStat(WograldStatsListener.CS_STAT_SPELL_DENY)) != 0) {
            ...
            if (level > 0) {
                ...
                if (sk != null && level <= sk.getLevel()) {
                    ...
                    if ((path & stats.getStat(WograldStatsListener.CS_STAT_SPELL_ATTUNE)) != 0) {
                        effective += 2;
                        supp = " (attuned)";
                    } else if ((path & stats.getStat(WograldStatsListener.CS_STAT_SPELL_REPEL)) != 0) {
                        effective -= 2;
                        supp = " (repelled)";
                    }
                    sb.append(effective);
