 ...
 */
public class DefaultPickmapManager<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends AbstractMapManager<G, A, R> {
     ...
     */
    public DefaultPickmapManager(@NotNull final MapReaderFactory<G, A> mapReaderFactory, @NotNull final MapControlFactory<G, A, R> mapControlFactory, @NotNull final GlobalSettings globalSettings, @NotNull final FaceObjectProviders faceObjectProviders) {
    ...
    protected MapControl<G, A, R> createMapControl(@Nullable final List<G> objects, @NotNull final A mapArchObject, @Nullable final File file) {
        return mapControlFactory.newPickmapControl(objects, mapArchObject, file);
    }
