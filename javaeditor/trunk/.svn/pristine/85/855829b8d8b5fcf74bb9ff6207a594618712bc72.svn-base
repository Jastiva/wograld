...
public class SysObjectNotOnLayerZeroChecker<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends AbstractValidator<G, A, R> implements GameObjectValidator<G, A, R> {
     ...
     */
    public SysObjectNotOnLayerZeroChecker(@NotNull final ValidatorPreferences validatorPreferences) {
    ...
    public void validateGameObject(@NotNull final G gameObject, @NotNull final ErrorCollector<G, A, R> errorCollector) {
        ...
        if (head.getAttributeInt(DefaultIsoGameObject.SYS_OBJECT, true) == 1 && head.getAttributeInt(DefaultIsoGameObject.LAYER, true) != 0) {
            errorCollector.collect(new SysObjectNotOnLayerZeroError<G, A, R>(gameObject));
        }
    }
