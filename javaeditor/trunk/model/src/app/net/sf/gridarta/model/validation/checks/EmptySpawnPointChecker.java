...
import net.sf.gridarta.model.validation.errors.EmptySpawnPointError;
 ...
 */
public class EmptySpawnPointChecker<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends AbstractValidator<G, A, R> implements GameObjectValidator<G, A, R> {
     ...
     */
    public EmptySpawnPointChecker(@NotNull final ValidatorPreferences validatorPreferences, @NotNull final Integer... typeNumbers) {
    ...
    public void validateGameObject(@NotNull final G gameObject, @NotNull final ErrorCollector<G, A, R> errorCollector) {
        if (typeNumbers.contains(gameObject.getTypeNo())) {
            if (gameObject.isEmpty()) {
                errorCollector.collect(new EmptySpawnPointError<G, A, R>(gameObject));
            }
        }
    }
