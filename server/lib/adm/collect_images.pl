#!/usr/bin/perl
#
# this script goes through and collects the various image sets into wograld-x.png
...
for ($count=0; $count<=$#extension; $count++) {
    ...
    $fh = $ESRV[$count];
    open($fh, ">wograld.$count") ||
	die("Can't open wograld.$count for write: $!\n");
	binmode( $fh );
}
...
while(<BMAPS>) {
	...
	if (open(FILE,"$filename")) {
		...
		push @csums, "$destfile.$file1 $sum wograld.$extension[$count]\@$position:$length\n";
	    } # if archive
	}
	elsif ($count==0) {
...
if ($archive) {
    ...
    print OUT "\$prefix/share/wograld-client.  \$prefix is the location given in the -prefix\n";
    print OUT "option when configure is run to build the client.  The default is /usr/local.\n";
    print OUT "In that case these files should be put in /usr/local/share/wograld-client\n";
    ...
    for ($count=0; $count<=$#extension; $count++) {
	system("cp wograld.$count $DESTDIR/wograld.$extension[$count]");
    }
