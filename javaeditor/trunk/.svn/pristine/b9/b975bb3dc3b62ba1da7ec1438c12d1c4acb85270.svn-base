...
public class DialogAttributeAnimationName<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends DialogAttribute<G, A, R, ArchetypeAttributeAnimationName> {

    /**
     * The input ui component for editing the value.
     */
    @NotNull
    private final AnimationComponent input;
     ...
     */
    public DialogAttributeAnimationName(@NotNull final ArchetypeAttributeAnimationName ref, @NotNull final AnimationComponent input) {
        super(ref);
        this.input = input;
    }
    ...
    public String getText2(@NotNull final G gameObject, @NotNull final Archetype<G, A, R> archetype, final String[] newMsg, final ArchetypeType archetypeType, final Component parent) {
        final String inline = input.getAnimName();
    ...
    public void appendSummary(@NotNull final Document doc, @NotNull final Style style) {
        final String value = input.getAnimName();
        if (value.length() > 0) {
            addLine(doc, style, getRef().getAttributeName() + " = " + value);
        }
    }

} // DialogAttributeAnimationName
