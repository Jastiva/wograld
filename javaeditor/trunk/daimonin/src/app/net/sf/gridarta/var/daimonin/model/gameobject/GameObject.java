...
/**
 * The <code>GameObject</code> class handles the Daimonin GameObjects. Usually,
...
public class GameObject extends DefaultIsoGameObject<GameObject, MapArchObject, Archetype> {
    ...
    /**
     * Maps values of {@link #GLOW_RADIUS} to effective glow radius in map
     * squares.
     */
    @NotNull
    private static final int[] LIGHT_MASK_WIDTH = { 0, 1, 2, 2, 3, 3, 3, 4, 4, 4, };
    ...
    public ImageIcon getImage(@NotNull final MapViewSettings mapViewSettings) {
        if (mapViewSettings.isAlphaType(getEditType())) {
            if (mapViewSettings.isDoubleFaces()) {
    ...
    public int getLightRadius() {
        final int attributeInt = Math.abs(getAttributeInt(GLOW_RADIUS));
        return LIGHT_MASK_WIDTH[Math.min(attributeInt, LIGHT_MASK_WIDTH.length - 1)];
    }
