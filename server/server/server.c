 ...
 *    "$Id: server.c,v 1.4 2012/09/28 21:14:01 serpentshard Exp $";
 */
...
void version(object *op) {
  ...
  new_draw_info_format(NDI_UNIQUE, 0, op, "This is Wograld v%s",VERSION);
...
void start_info(object *op) {
  ...
  sprintf(buf,"Welcome to Wograld, v%s!",VERSION);
...
printf("%s\n","judge4");
mapstruct *mp;
for(mp=first_map;mp!=NULL;mp=mp->next)
{
	printf("%s\n",mp->path);

	if(mp->upper)
	{
		printf("%s\n","this is above the former");
		printf("%s\n",mp->upper);
		printf("%s\n","this was found above the former");
	}

	if(mp->lower)
	{
		printf("%s\n","this is under the former");
		printf("%s\n",mp->lower);
		printf("%s\n","this was found under the former");
	}
...
void fall_enter(object *faller, mapstruct *m)
{	
	enter_map(faller, m, faller->x, faller->y);
}
...
int server_main(int argc, char **argv)
{
  ...
  for(;;) {
    ...
    cftimer_process_timers();/* Process the wograld Timers */    
