...
/**
 * Insert right below the bottommost floor square. Inserts bottommost if no
 * floor square exists.
 * @author Andreas Kirschbaum
 */
public class BelowFloorInsertionMode<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> implements InsertionMode<G, A, R> {
     ...
     */
    public BelowFloorInsertionMode(@Nullable final GameObjectMatcher floorGameObjectMatcher) {
    ...
    public void insert(@NotNull final G gameObject, @NotNull final MapSquare<G, A, R> mapSquare) {
        ...
        final G firstFloor = mapSquare.getFirst(floorGameObjectMatcher);
        if (firstFloor == null) {
            mapSquare.addFirst(gameObject);
            return;
        }

        mapSquare.insertAfter(firstFloor, gameObject);
    }
