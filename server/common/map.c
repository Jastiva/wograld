 ...
 *   "$Id: map.c,v 1.4 2011/05/21 20:11:07 serpentshard Exp $";
 */
...
/*
 * This makes a path absolute outside the world of Wograld.
  ...
  m->tracknum=13;
	...
	if (check_path(value, 1) != -1) {
		...
		if (path != NULL) {
			...
			char *value2 = strdup(path);
			char value3[70];
			int vvv;
			for (vvv = 0; vvv < strlen(value2); vvv++)
			{
				if(isgraph(value2[vvv]))
				{
					value3[vvv] = value2[vvv];
				}
				else
				{
					break;
	...
	if (path != NULL) {
		...
		char *value4 = strdup(path);
			char value5[70];
			int vvv;
			for (vvv = 0; vvv < strlen(value4); vvv++)
			{
				if(isgraph(value4[vvv]))
				{
					value5[vvv] = value4[vvv];
				}
				else
				{
					break;
...
void free_map(mapstruct *m,int flag) {
    ...
    if (m->upper) FREE_AND_CLEAR(m->upper);
    if (m->lower) FREE_AND_CLEAR(m->lower);
    m->up_map = NULL;
    m->down_map = NULL;
    m->in_memory = MAP_SWAPPED;
}
...
printf("%s %s\n","about to get tiled map at",path);
    orig_map->tile_map[tile_num] = ready_map_name(path, 0);
printf("%s %s\n","just opened tiled map at",path);
...
mapstruct *load_and_link_upper_map(mapstruct *orig_map)
{
   
    char *path = path_combine_and_normalize(orig_map->path, orig_map->upper);

    orig_map->up_map = ready_map_name(path, 0);


    if (orig_map->up_map->lower &&
	!strcmp(orig_map->up_map->lower, orig_map->path))
	orig_map->up_map->down_map = orig_map;

    return orig_map->up_map;
}

mapstruct *load_and_link_lower_map(mapstruct *orig_map)
{
   
    char *path = path_combine_and_normalize(orig_map->path, orig_map->lower);

    orig_map->down_map = ready_map_name(path, 0);

    /* need to do a strcmp here as the orig_map->path is not a shared string */
    if (orig_map->down_map->upper &&
	!strcmp(orig_map->down_map->upper, orig_map->path))
	orig_map->down_map->up_map = orig_map;

    return orig_map->down_map;
}
 ...
 * can be negativbe.  direction is the wograld direction scheme
