 ...
 *   "$Id: init.c,v 1.3 2011/08/10 20:19:14 serpentshard Exp $";
 */
...
void init_environ(void) {
    ...
    cp=getenv("WOGRALD_LIBDIR");
    if (cp) settings.datadir=cp;
    cp=getenv("WOGRALD_LOCALDIR");
    if (cp) settings.localdir=cp;
    cp=getenv("WOGRALD_PLAYERDIR");
    if (cp) settings.playerdir=cp;
    cp=getenv("WOGRALD_MAPDIR");
    if (cp) settings.mapdir=cp;
    cp=getenv("WOGRALD_ARCHETYPES");
    if (cp) settings.archetypes=cp;
    cp=getenv("WOGRALD_TREASURES");
    if (cp) settings.treasures=cp;
    cp=getenv("WOGRALD_UNIQUEDIR");
    if (cp) settings.uniquedir=cp;
    cp=getenv("WOGRALD_TEMPLATEDIR");
    if (cp) settings.templatedir=cp;
    cp=getenv("WOGRALD_TMPDIR");
