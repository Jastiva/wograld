 ...
 *   "$Id: player.c,v 1.8 2014/12/13 22:03:00 serpentshard Exp $";
 */
...
int add_player(socket_struct *ns) {
    ...
    player *q;
    int dupl = 0;
	for(q=first_player; q!= NULL; q=q->next){
		if(q->socket.ident == ns->ident)
		{  
			dupl = 1;
			break;
		}
    ...
    return 0;
	/* the check above is supposed to prevent a single client from connecting twice, in which case */
	/* the client supposedly has already been added successfully; when connection is initialized, */
	/* a number is written to the ns and incremented, then when client attempts to add a player, */
	/* this number is looked for in the sockets of all existing players, and a player is created with */
	/* that socket, only if an existing player does not already have a socket with that identifier */
	/* and as it is desirable to be able to have more than one client per ip, ip is not what is used */
	/* during gameplay, the status of the socket may become dead, or the client may leave, in which case */
	/* the player data is removed; and while the identifier remains on the init socket, this is not one of the */
	/* sockets associated with a player, and furthermore it will be overwritten by a new identifier if any more */
	/* connections attempt to use it; although the value of ident may wraparound, low collision chance */
...
/* start new code */
 archetype *at = find_archetype("close_bank");
   object *close_b = arch_to_object(at);
   op->bank = close_b;
   close_b->env = op;
...
/* end new code */
 ...
 /* if(*first_map_ext_path) { */
            ...
            snprintf(mapname, MAX_BUF-1, "%s/%s",
              "start/HallsOfSelection", op->arch->name);
            printf("%s\n", mapname);
            tmp=get_object();
...
/*  hacked out some or all of the stat loss 10/11/11 */
...
char buf[MAX_BUF];
 ...
 if (settings.not_permadeth == TRUE) {
    ...
    if(settings.wait_res_or_bed == FALSE) {
...
/*              const char *god = determine_god(op);
 ...
 cure_disease(op,0);  /* remove any disease */
   ...
   object *force;
       ...
       } // wait_res_or_bed false
       else
       {
  
        // new code, leave a corpse and loot
        // and give pl the option to go to savebed

        // do not drop quests or skills or spells
        // this is already accounted for by loot_player

        
       op->contr->waitforres = 1;
      ...
      // during this mode
      // which does not cause pl to leave ST_PLAYING
      // but requires filtering of player commands

      // to only permit them to go to savebed or wait
      // or to disconnect
      // and does not permit movement
 
      // but unlike permadeth
 ...
 //     op->stats.hp=op->stats.maxhp;
	...
	remove_ob(op);
      ...
      // do not stuck the server, do other stuff while wait for client events

  new_draw_info(NDI_UNIQUE, 0,op,"YOU HAVE DIED.");


   new_draw_info( NDI_UNIQUE, 0, op, "If you do not want to wait for someone to resurrect," );
   new_draw_info( NDI_UNIQUE, 0, op, "Then type go_savebed" );  
        return;
       }
    } /* NOT_PERMADETH */
    else {
...
void ranger_ability_gain(object *who, int level) {
	...
	trlist = find_treasurelist("ranger_skills");
   ...
   // for (i=0, tr = trlist->items; tr != NULL && i<level-1;
    ...
    for(tr=trlist->items; tr != NULL; tr=tr->next)
    {
     ...
     /*
         if(tr != NULL)
         {
               if(tr->item != NULL)
               {
                     if(!check_spell_known(who, tr->item->name))
                     {
                           break;
                     }
               }
	...
	return;
         ...
         new_draw_info_format(NDI_UNIQUE,0,who,"trying to get spell of level %i\n", tr->magic);
...
void knight_ability_gain(object *who, int level) {
	...
	trlist = find_treasurelist("knight_skills");
