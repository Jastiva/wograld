...
dnl CF_CHECK_WOGEDIT(X11LIBS, ACTION-IF-FOUND , ACTION-IF-NOT-FOUND)
dnl check if a sample X test code can compile
dnl

AC_DEFUN([CF_CHECK_WOGEDIT],[

    ac_save_LIBS="$LIBS"

    LIBS="$1 $LIBS" 
    CFLAGS="-Iinclude -Iwogedit/include"
    AC_MSG_CHECKING(for wogedit required headers)
    AC_TRY_LINK([
#include <Posix.h>
#include <Xaw.h>
#include <Ansi.h>
