char *rcsid_gtk2_keys_c =
    "$Id: keys.c,v 1.1.1.1 2012/09/14 03:30:02 serpentshard Exp $";
/*
...
void keys_init(GtkWidget *window_root)
{
    ...
    if ( strlen( cpl.name ) )
        sprintf( buf, "%s/.wograld/%s.keys", getenv( "HOME" ), cpl.name );
    else
	...
	LOG(LOG_INFO,"gtk::init_keys","Could not open ~/.wograld/keys, trying to load global bindings");
