...
<!-- $Header: /cvsroot/wograld/javaeditor/trunk/src/doc/xhtml2html.xslt,v 1.1.1.1 2013/05/29 19:04:31 serpentshard Exp $ -->
<xsl:transform version="2.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:xml="http://www.w3.org/XML/1998/namespace" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="html" xmlns:saxon="http://saxon.sf.net/">

    <!-- Ausgabe: HTML 4.01 / UTF-8 -->
    <xsl:output method="html" doctype-public="-//W3C//DTD HTML 4.01//EN" doctype-system="http://www.w3.org/TR/html4/strict.dtd" indent="no" encoding="utf-8" media-type="text/html" omit-xml-declaration="yes" saxon:character-representation="native;hex"/>
    ...
    <xsl:template match="processing-instruction('php')">
        <xsl:processing-instruction name="php"><xsl:value-of select="."/>?
        </xsl:processing-instruction>
    </xsl:template>
    <xsl:template match="processing-instruction('cgi')">
        <xsl:processing-instruction name="cgi"><xsl:value-of select="."/>?
        </xsl:processing-instruction>
    </xsl:template>
    ...
    <xsl:template match="html:img[@src='http://www.w3.org/Icons/valid-xhtml11' and @alt='Valid XHTML 1.1']">
        <xsl:element name="{local-name()}">
            <xsl:apply-templates select="@*"/>
            <xsl:attribute name="src">http://www.w3.org/Icons/valid-html401
            </xsl:attribute>
