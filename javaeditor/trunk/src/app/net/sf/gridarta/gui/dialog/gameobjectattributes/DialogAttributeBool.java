...
public class DialogAttributeBool<G extends GameObject<G, A, R>, A extends MapArchObject<A>, R extends Archetype<G, A, R>> extends DialogAttribute<G, A, R, ArchetypeAttributeBool> {
     ...
     */
    public DialogAttributeBool(@NotNull final ArchetypeAttributeBool ref, @NotNull final AbstractButton input) {
        super(ref);
        this.input = input;
    }
    ...
    public String getText2(@NotNull final G gameObject, @NotNull final Archetype<G, A, R> archetype, final String[] newMsg, final ArchetypeType archetypeType, final Component parent) {
        ...
        if (input.isSelected() != (archetype.getAttributeInt(archetypeAttributeName) == 1)) {
            return archetypeAttributeName + " " + (input.isSelected() ? 1 : 0);
        }
